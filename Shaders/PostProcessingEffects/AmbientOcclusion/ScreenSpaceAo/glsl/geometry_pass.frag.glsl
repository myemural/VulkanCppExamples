...
void main()
{
    ...
    // Scaled UV
    vec2 scaledUv = fragUv * meshMaterial.uvScale;

    vec4 albedo = meshMaterial.diffuseColor.rgba;
    if (meshMaterial.diffuseMap != -1) {
        vec4 diffuseTextureColor = texture(uCombinedSamplers[nonuniformEXT(meshMaterial.diffuseMap)], scaledUv);
