...
#define TEXTURE_COUNT 2 // Workaround: Getting array size from specialization constants is not working on HLSL
...
float4 main(PSInput input) : SV_Target
{
    ...
    // Opacity calculation
    float opacity = meshInfo.opacity;
    if (meshInfo.opacityMap != -1) {
        float minOpacity = 0.0;
        float maxOpacity = 1.0;

        float opacityValue = uImages[meshInfo.opacityMap].Sample(uSamplers[meshInfo.opacityMap], input.fragUv).r;
        opacity = lerp(minOpacity, maxOpacity, opacityValue);
    }
