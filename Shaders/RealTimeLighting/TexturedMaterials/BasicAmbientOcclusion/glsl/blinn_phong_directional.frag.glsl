...
void main()
{
    ...
    float ao = 1.0f;
    if (meshMatInfo.aoMap != -1) {
        ao = texture(uCombinedSamplers[nonuniformEXT(meshMatInfo.aoMap)], fragUv).r;
    }
    ...
    // Ambient calculation
    vec3 ambient = meshMatInfo.ambientStrength * ao * light.lightAmbientColor.rgb;
