...
float4 main(PSInput input) : SV_Target
{
    ...
    if (meshInfo.emissiveMap != -1) {
        emissiveColor = uImages[meshInfo.emissiveMap].Sample(uSamplers[meshInfo.emissiveMap], input.fragUv).rgb;
    }

    // Final color
    float3 finalColor = ambient + diffuse + specular + emissiveColor;
