...
[[vk::binding(4, 0)]] SamplerState uPlanarReflectionMapSampler;
[[vk::binding(4, 0)]] Texture2D uPlanarReflectionMapTexture;
...
float4 main(PSInput input) : SV_Target
{
    ...
    // World-space fragment position to reflection texture UV
    float4 projCoord = mul(pc.reflectionViewProj, float4(input.fragPos, 1.0));
    ...
    if (projCoord.w > 0.0) {
        ...
        if (uv.x >= 0.0 && uv.x <= 1.0 && uv.y >= 0.0 && uv.y <= 1.0) {
            float3 reflectionColor = uPlanarReflectionMapTexture.Sample(uPlanarReflectionMapSampler, uv).rgb;
            finalColor = lerp(finalColor, reflectionColor, meshInfo.reflectivity);
        }
    }
