...
void main()
{
    ...
    // Opacity calculation
    float opacity = meshMatInfo.opacity;
    if (meshMatInfo.opacityMap != -1) {
        float minOpacity = 0.0;
        float maxOpacity = 1.0;

        float opacityValue = texture(uCombinedSamplers[nonuniformEXT(meshMatInfo.opacityMap)], fragUv).r;
        opacity = mix(minOpacity, maxOpacity, opacityValue);
    }
