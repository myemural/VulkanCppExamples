...
float2 ParallaxOcclusionMapping(float2 texCoords, float3 viewDirTS, float heightScale)
{
    ...
    float2 P = (viewDirTS.xy / viewDirZ) * heightScale * parallaxFade;
    ...
    float currentHeight = uImages[heightMapIndex].Sample(uSamplers[heightMapIndex], currentTexCoords).r;
    ...
    while (rayHeight > currentHeight)
    {
        ...
        rayHeight -= layerStep;
        currentHeight = uImages[heightMapIndex].Sample(uSamplers[heightMapIndex], currentTexCoords).r;
    }
...
float4 main(PSInput input) : SV_Target
{
    ...
    if (meshInfo.heightMap != -1) {
        ...
        // UV offset
        uv = ParallaxOcclusionMapping(input.fragUv, viewDirTangentSpace, parallaxScale);
    }
