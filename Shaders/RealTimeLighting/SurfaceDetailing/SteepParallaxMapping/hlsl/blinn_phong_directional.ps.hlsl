...
float2 SteepParallaxMapping(float2 texCoords, float3 viewDirTS, float heightScale)
{
    ...
    float sampledHeight = uImages[heightMapIndex].Sample(uSamplers[heightMapIndex], currentTexCoords).r;

    while (rayHeight > sampledHeight)
    {
        currentTexCoords -= deltaTexCoords;
        rayHeight -= layerStep;
        sampledHeight = uImages[heightMapIndex].Sample(uSamplers[heightMapIndex], currentTexCoords).r;
    }
...
float4 main(PSInput input) : SV_Target
{
    ...
    if (meshInfo.heightMap != -1) {
        ...
        // UV offset
        uv = SteepParallaxMapping(input.fragUv, viewDirTangentSpace, parallaxScale);
    }
