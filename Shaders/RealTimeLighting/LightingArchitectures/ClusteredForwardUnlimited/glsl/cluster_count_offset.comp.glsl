...
layout(std430, set = 0, binding = 4) buffer ClusterHeaderBuffer
{
    ClusterHeader clusterHeaders[];
};

/// NOTE: This is not used in this shader.
layout(std430, set=0, binding = 5) readonly buffer GlobalLightIndexBuffer
{
    uint globalLightIndices[];
};

layout(std430, set=0, binding = 6) buffer GlobalCounterBuffer
{
    uint globalLightRefCount;
};
...
void main()
{
    ...
    if(gl_LocalInvocationIndex == 0) {
        uint offset = atomicAdd(globalLightRefCount, localCount);
