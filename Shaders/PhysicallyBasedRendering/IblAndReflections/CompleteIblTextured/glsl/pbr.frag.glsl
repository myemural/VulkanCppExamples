...
void main()
{
    ...
    // If metallic/roughness map texture exist, apply it
    if (meshInfo.metallicRoughnessMap != -1) {
        roughness = texture(uCombinedSamplers[nonuniformEXT(meshInfo.metallicRoughnessMap)], fragUv).g;
        metallic = texture(uCombinedSamplers[nonuniformEXT(meshInfo.metallicRoughnessMap)], fragUv).b;
    }
    ...
    vec3 ambient = (diffuseIBL + specularIBL) * ambientOcclusion;

    // Emissive light calculation
    vec3 emissive = vec3(0.0);
    if (meshInfo.emissiveMap != -1)
    {
        emissive = texture(uCombinedSamplers[nonuniformEXT(meshInfo.emissiveMap)], fragUv).rgb;
    }

    // Final outgoing light
    vec3 Lo = emissive + directLighting + ambient;
