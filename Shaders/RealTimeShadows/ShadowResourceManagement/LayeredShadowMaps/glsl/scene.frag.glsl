...
layout(set = 0, binding = 4) uniform sampler2DArray uShadowMaps;
...
float calculateShadow(mat4 lightSpaceMatrix, float lightIndex, vec3 normalWorldSpace, vec3 normalizedLightDir)
{
    ...
    float closestDepth = texture(uShadowMaps, vec3(shadowUV, lightIndex)).r;
...
vec3 calculateSpotLight(LightData lightData, int lightIndex, vec3 normalWorldSpace, vec3 fragmentPosition)
{
    // Get mesh info
    ...
    // Final color
    float shadow = calculateShadow(lightData.lightSpaceMatrix, float(lightIndex), normalWorldSpace, normalizedLightDir);
    vec3 finalColor = ambient + (1.0 - shadow) * spotFactor * (diffuse + specular);
    return finalColor;
}

vec3 calculateDirectionalLight(LightData lightData, int lightIndex, vec3 normalWorldSpace, vec3 fragmentPosition)
{
    // Get mesh info
    const MeshMaterialData meshMatInfo = meshMaterials[pc.objectId];
...
void main()
{
    ...
    for (int i = 0; i < NUM_OF_LIGHTS; ++i)
    {
        if (uint(light.lightData[i].lightParams.x) == 0) {
            resultColor += calculateDirectionalLight(light.lightData[i], i, normalWorldSpace, fragPos);
        } else {
            resultColor += calculateSpotLight(light.lightData[i], i, normalWorldSpace, fragPos);
        }
    }
