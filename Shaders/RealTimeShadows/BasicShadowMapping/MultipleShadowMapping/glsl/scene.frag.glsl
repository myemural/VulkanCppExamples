...
{
    vec3 fragToLight = fragPos - light.pointLightPosition.xyz;
    float currentDepth = length(fragToLight);

    float closestDepth = texture(uPointLightCubemapShadowMap, fragToLight).r * pc.farPlane;
...
vec3 calculatePointLight(vec3 normalWorldSpace, vec3 fragmentPosition)
{
    // Get mesh info
    const MeshMaterialData meshMatInfo = meshMaterials[pc.objectId];

    // Normalizing light direction
    vec3 normalizedLightDir = normalize(light.pointLightPosition.xyz - fragmentPosition);
    ...
    vec3 diffuse = diff * light.pointLightColor.rgb * diffuseColor;
    ...
    vec3 specular = meshMatInfo.specularStrength * spec * light.pointLightColor.rgb * meshMatInfo.specularColor.rgb;

    // Attenuation calculation
    float distance = length(light.pointLightPosition.xyz - fragPos);
...
float calculateSpotLightShadow(vec3 normalWorldSpace, vec3 normalizedLightDir)
{
    vec4 fragPosLightSpace = light.spotLightSpaceMatrix * vec4(fragPos,1.0);
    ...
    float closestDepth = texture(uSpotLightShadowMap, shadowUV).r;
...
vec3 calculateSpotLight(vec3 normalWorldSpace, vec3 fragmentPosition)
{
    // Get mesh info
    const MeshMaterialData meshMatInfo = meshMaterials[pc.objectId];

    // Normalizing light direction
    vec3 normalizedLightDir = normalize(light.spotLightPosition.xyz - fragmentPosition);
    ...
    // Diffuse (Lambert) calculation
    float diff = max(dot(normalWorldSpace, normalizedLightDir), 0.0);
    vec3 diffuse = diff * light.spotLightColor.rgb * diffuseColor;

    // Specular calculation
    vec3 halfDir = normalize(normalizedLightDir + normalizedView);
    float spec = pow(max(dot(normalWorldSpace, halfDir), 0.0), meshMatInfo.shininess);
    vec3 specular = meshMatInfo.specularStrength * spec * light.spotLightColor.rgb * meshMatInfo.specularColor.rgb;

    // Spotlight contribution calculation (soft-cutoff)
    vec3 spotDir = normalize(-light.spotLightDirection.xyz);
    ...
    // Final color
    float shadow = calculateSpotLightShadow(normalWorldSpace, normalizedLightDir);
    vec3 finalColor = ambient + (1.0 - shadow) * spotFactor * (diffuse + specular);
    return finalColor;
}

float calculateDirectionalLightShadow(vec3 normalWorldSpace, vec3 normalizedLightDir)
{
    vec4 fragPosLightSpace = light.directionalLightSpaceMatrix * vec4(fragPos,1.0);
    vec3 projCoords = fragPosLightSpace.xyz / fragPosLightSpace.w;
    projCoords = projCoords * 0.5 + 0.5;
    ...
    float closestDepth = texture(uDirLightShadowMap, shadowUV).r;
    closestDepth = closestDepth * 0.5 + 0.5;
...
vec3 calculateDirectionalLight(vec3 normalWorldSpace, vec3 fragmentPosition)
{
    // Get mesh info
    const MeshMaterialData meshMatInfo = meshMaterials[pc.objectId];

    // Normalizing light direction
    vec3 normalizedLightDir = normalize(-light.directionalLightDirection.xyz);
    ...
    // Final color
    float shadow = calculateDirectionalLightShadow(normalWorldSpace, normalizedLightDir);
    vec3 finalColor = ambient + (1.0 - shadow) * (diffuse + specular);
    return finalColor;
}
...
void main()
{
    ...
    resultColor += calculatePointLight(normalWorldSpace, fragPos);
    resultColor += calculateSpotLight(normalWorldSpace, fragPos);
    resultColor += calculateDirectionalLight(normalWorldSpace, fragPos);
