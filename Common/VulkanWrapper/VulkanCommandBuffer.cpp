...
bool VulkanCommandBuffer::BeginCommandBuffer(
    const std::function<void(VkCommandBufferBeginInfo &)> &beginInfoCallback) const
{
                                             ...
                                             const std::vector<std::uint32_t> &dynamicOffsets) const
{
    ...
    vkCmdBindDescriptorSets(handle_, pipelineBindPoint, pipelineLayout->GetHandle(), firstSet,
                            descriptorSets.size(), vkDescriptorSets.empty() ? nullptr : vkDescriptorSets.data(),
...
void VulkanCommandBuffer::Draw(const std::uint32_t vertexCount, const std::uint32_t instanceCount,
                               const std::uint32_t firstVertex, const std::uint32_t firstInstance) const
{
    vkCmdDraw(handle_, vertexCount, instanceCount, firstVertex, firstInstance);
}

void VulkanCommandBuffer::DrawIndexed(const std::uint32_t indexCount, const std::uint32_t instanceCount,
                                      const std::uint32_t firstIndex, const std::int32_t vertexOffset,
...
{
    vkCmdPipelineBarrier(handle_, srcStage, dstStage, dependencyFlags,
                         memoryBarriers.size(),
