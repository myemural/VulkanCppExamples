...
void VulkanApplication::CreateInitialResources() const
{
    ...
    resourceCreateInfo.images = {
        ImageResourceCreateInfo{
            .name = kPointLightShadowCubemapImage,
            ...
            .views =
                    {
                        ImageViewCreateInfo{.viewName = kPointLightShadowCubemapImageView,
                                            .viewType = VK_IMAGE_VIEW_TYPE_CUBE,
                                            .format = depthImageFormat_,
                                            .subresourceRange{VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 0, 6}},
                        ImageViewCreateInfo{.viewName = kPointLightShadowCubemapImageViewRight,
                                            .viewType = VK_IMAGE_VIEW_TYPE_2D,
                                            .format = depthImageFormat_,
                                            .subresourceRange{VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 0, 1}},
                        ImageViewCreateInfo{.viewName = kPointLightShadowCubemapImageViewLeft,
                                            .viewType = VK_IMAGE_VIEW_TYPE_2D,
                                            .format = depthImageFormat_,
                                            .subresourceRange{VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 1, 1}},
                        ImageViewCreateInfo{.viewName = kPointLightShadowCubemapImageViewTop,
                                            .viewType = VK_IMAGE_VIEW_TYPE_2D,
                                            .format = depthImageFormat_,
                                            .subresourceRange{VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 2, 1}},
                        ImageViewCreateInfo{.viewName = kPointLightShadowCubemapImageViewBottom,
                                            .viewType = VK_IMAGE_VIEW_TYPE_2D,
                                            .format = depthImageFormat_,
                                            .subresourceRange{VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 3, 1}},
                        ImageViewCreateInfo{.viewName = kPointLightShadowCubemapImageViewBack,
                                            .viewType = VK_IMAGE_VIEW_TYPE_2D,
                                            .format = depthImageFormat_,
                                            .subresourceRange{VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 4, 1}},
                        ImageViewCreateInfo{.viewName = kPointLightShadowCubemapImageViewFront,
                                            .viewType = VK_IMAGE_VIEW_TYPE_2D,
                                            .format = depthImageFormat_,
...
void VulkanApplication::BuildScene()
{
    ...
    pointLightCamera_ = std::make_shared<PerspectiveCamera>(glm::vec3(0.0f), 1.0f, 90.0f, 0.1f, kPointLightFarPlane);
    ...
    const auto& rootObject = SceneObjectBuilder(*scene_, kRootObject)
                                     .AddChild(SceneObjectBuilder(*scene_, kObject + std::to_string(index++))
                                                       ...
                                                       .WithScale(glm::vec3{1.5f}))
                                     .AddChild(SceneObjectBuilder(*scene_, kObject + std::to_string(index++))
                                                       .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                                       .WithMaterial(objectMaterial)
                                                       .WithPosition(glm::vec3(0.0f, 1.0f, -5.0f)))
                                     .AddChild(SceneObjectBuilder(*scene_, kObject + std::to_string(index++))
                                                       .WithBuiltinMesh(BuiltinMeshType::CONE)
                                                       .WithMaterial(objectMaterial)
                                                       .WithPosition(glm::vec3(4.0f, -2.0f, 0.0f))
                                                       .WithScale(glm::vec3{1.5f}))
                                     .AddChild(SceneObjectBuilder(*scene_, kObject + std::to_string(index++))
                                                       .WithBuiltinMesh(BuiltinMeshType::CONE)
                                                       .WithMaterial(objectMaterial)
                                     ...
                                     .AddChild(SceneObjectBuilder(*scene_, kObject + std::to_string(index++))
                                                       ...
                                                       .WithPosition(glm::vec3(0.0f, 0.0f, -8.0f))
                                                       .WithEulerAngles(glm::vec3(90.0f, 0.0f, 0.0f))
                                                       .WithScale(glm::vec3{9.0f}))
                                     .AddChild(SceneObjectBuilder(*scene_, kObject + std::to_string(index))
                                                       .WithBuiltinMesh(BuiltinMeshType::PLANE)
                                                       .WithMaterial(planeMaterial)
                                                       .WithPosition(glm::vec3{0.0f, -5.0f, 0.0f})
                                                       .WithScale(glm::vec3{20.0f}))
                                     .AddChild(SceneObjectBuilder(*scene_, kPointLightObject)
                                                       .WithTag(kLightGroup)
                                                       .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                                       .WithMaterial(Material{})
                                                       .WithPosition(glm::vec3(0.0f, 6.0f, 0.0f))
                                                       .WithScale(glm::vec3{0.3f}))
                                     .AddChild(SceneObjectBuilder(*scene_, kSpotLightObject)
                                                       .WithTag(kLightGroup)
                                                       .WithBuiltinMesh(BuiltinMeshType::CONE)
                                                       .WithMaterial(Material{})
                                                       .WithPosition(glm::vec3(0.0f, 3.0f, 0.0f))
                                                       .WithScale(glm::vec3{0.3f}))
                                     .Build();
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        .maxSets = 4 + combinedImageSamplerCount + 3,
    ...
    std::vector<VkDescriptorImageInfo> pointLightShadowMapImageInfos;
    pointLightShadowMapImageInfos.emplace_back(
            resources_->GetSampler(kShadowSampler)->GetHandle(),
            resources_->GetImageView(kPointLightShadowCubemapImage, kPointLightShadowCubemapImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    std::vector<VkDescriptorImageInfo> dirLightShadowMapImageInfos;
    dirLightShadowMapImageInfos.emplace_back(
            resources_->GetSampler(kShadowSampler)->GetHandle(),
            resources_->GetImageView(kDirLightShadowMapImage, kDirLightShadowMapImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    std::vector<VkDescriptorImageInfo> spotLightShadowMapImageInfos;
    spotLightShadowMapImageInfos.emplace_back(
            resources_->GetSampler(kShadowSampler)->GetHandle(),
            resources_->GetImageView(kSpotLightShadowMapImage, kSpotLightShadowMapImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    ImageWriteRequest pointShadowMapTextureUpdateRequest;
    pointShadowMapTextureUpdateRequest.descriptorSetName = kMainDescSet;
    pointShadowMapTextureUpdateRequest.bindingIndex = 4;
    pointShadowMapTextureUpdateRequest.images = pointLightShadowMapImageInfos;
    pointShadowMapTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest dirShadowMapTextureUpdateRequest;
    dirShadowMapTextureUpdateRequest.descriptorSetName = kMainDescSet;
    dirShadowMapTextureUpdateRequest.bindingIndex = 5;
    dirShadowMapTextureUpdateRequest.images = dirLightShadowMapImageInfos;
    dirShadowMapTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest spotShadowMapTextureUpdateRequest;
    spotShadowMapTextureUpdateRequest.descriptorSetName = kMainDescSet;
    spotShadowMapTextureUpdateRequest.bindingIndex = 6;
    spotShadowMapTextureUpdateRequest.images = spotLightShadowMapImageInfos;
    spotShadowMapTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    ...
    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        ...
        .imageWriteRequests = {textureUpdateRequest, pointShadowMapTextureUpdateRequest,
                               dirShadowMapTextureUpdateRequest, spotShadowMapTextureUpdateRequest}};
...
{
    // Shadow map framebuffer (for point light)
    const std::vector<std::string> cubemapImageViewKeys = {
        kPointLightShadowCubemapImageViewRight, kPointLightShadowCubemapImageViewLeft,
        kPointLightShadowCubemapImageViewTop,   kPointLightShadowCubemapImageViewBottom,
        kPointLightShadowCubemapImageViewBack,  kPointLightShadowCubemapImageViewFront};

    for (auto i = 0U; i < cubemapImageViewKeys.size(); ++i) {
        const auto& shadowCubemapImageView =
                resources_->GetImageView(kPointLightShadowCubemapImage, cubemapImageViewKeys[i]);
    ...
    // Shadow map framebuffer (for directional light)
    const auto& dirLightShadowDepthImageView =
    ...
    // Shadow map framebuffer (for spotlight)
    const auto& spotLightShadowDepthImageView =
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    const auto cubemapViewMatrices = pointLightCamera_->GetCubemapViewMatrices();
    auto cubemapProj = pointLightCamera_->GetProjectionMatrix();
    cubemapProj[1][1] *= -1; // Revert this projection flip for shadow map render

    // Create shadow map (for point light) pass
    for (auto i = 0U; i < pointLightShadowFramebuffers_.size(); ++i) {
        ...
        scene_->Traverse([&](const SceneObject& sceneObject) {
            if (sceneObject.HasRenderable() && sceneObject.GetTag() != kLightGroup) {
                ...
                shadowMapPushConstant.lightPos =
                        glm::vec4(scene_->FindObjectByName(kPointLightObject)->GetPosition(), 1.0f);
                shadowMapPushConstant.farPlane = kPointLightFarPlane;
                shadowMapPushConstant.isPointLightShadow = 1;
    ...
    // Create shadow map (for directional light) pass
    {
        currentCmdBuffer->BeginRenderPass(
                [&](auto& beginInfo) {
                    beginInfo.renderPass = shadowRenderPass_->GetHandle();
    ...
    // Create shadow map (for spotlight) pass
    {
        currentCmdBuffer->BeginRenderPass(
                [&](auto& beginInfo) {
                    beginInfo.renderPass = shadowRenderPass_->GetHandle();
        ...
        scene_->Traverse([&](const SceneObject& sceneObject) {
            if (sceneObject.HasRenderable() && sceneObject.GetTag() != kLightGroup) {
                ...
                const glm::mat4 lightProjection = spotLightCamera_->GetProjectionMatrix();
                const glm::mat4 lightView = spotLightCamera_->GetViewMatrix();
                shadowMapPushConstant.lightSpaceMatrix = lightProjection * lightView;
                currentCmdBuffer->PushConstants(shadowPipelineLayout_,
                                                VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                                sizeof(shadowMapPushConstant), &shadowMapPushConstant);
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    scene_->FindObjectByName(kPointLightObject)->SetPosition(pos);

    pointLightCamera_->SetPosition(pos);
    ...
    const glm::mat4 spotLightView = spotLightCamera_->GetViewMatrix();
