...
namespace examples::real_time_shadows::shadow_resource_management::mipmapped_shadow_maps
{
...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto shadowMapMipLevels = GetMipLevelCount(kShadowMapSize, kShadowMapSize);
    ...
    resourceCreateInfo.images = {
        ImageResourceCreateInfo{
            ...
            .views = {ImageViewCreateInfo{.viewName = kShadowMapImageViewMip0,
                                          .format = VK_FORMAT_R32G32_SFLOAT,
                                          .subresourceRange = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
                                                               ...
                                                               .baseArrayLayer = 0,
                                                               .layerCount = 1}},
                      ImageViewCreateInfo{.viewName = kShadowMapImageView,
                                          .format = VK_FORMAT_R32G32_SFLOAT,
                                          .subresourceRange = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
                                                               .baseMipLevel = 0,
...
void VulkanApplication::BuildScene()
{
    ...
    lightCamera_ = std::make_shared<OrthographicCamera>(glm::vec3(0.0f), 1.0f, 80.0f, 0.1f, 50.0f);
    ...
    for (auto i = 0; i < 20; ++i) {
        ...
        rootObjectBuilder
                .AddChild(SceneObjectBuilder(*scene_, kCylinderObject + indexStr) // Left pillars
                                  ...
                                  .WithPosition(glm::vec3{7.5f, 1.0f, zShift})
    ...
    const auto& rootObject =
            rootObjectBuilder
                    ...
                    .AddChild(SceneObjectBuilder(*scene_, kCubeObject)
                                      ...
                                      .WithPosition(glm::vec3{0.0f, -1.0f, -10.0f})
                    ...
                    .AddChild(SceneObjectBuilder(*scene_, kFloorObject)
                                      ...
                                      .WithPosition(glm::vec3{0.0f, -2.0f, -7.0f})
                                      .WithScale(glm::vec3{30.0f, 1.0f, 50.0f}))
                    .Build();
...
void VulkanApplication::CreateFramebuffers()
{
    ...
    const auto& shadowMapImageView = resources_->GetImageView(kShadowMapImage, kShadowMapImageViewMip0);
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Generate mipmaps for shadow map image
    const auto shadowMapMipLevels = GetMipLevelCount(kShadowMapSize, kShadowMapSize);
    {
        resources_->GenerateMipmaps(currentCmdBuffer, kShadowMapImage, kShadowMapSize, kShadowMapSize,
                                    shadowMapMipLevels, 1);
    }
    ...
    // Draw scene pass
    {
        ...
        scene_->Traverse([&](const SceneObject& sceneObject) {
            if (sceneObject.HasRenderable()) {
                ...
                scenePushConstant.cameraFarPlane = camera_->GetFarPlane();
                scenePushConstant.enableShadowMapLod = isShadowMapLodEnabled_ ? 1U : 0U;
                scenePushConstant.shadowMapMaxLod = shadowMapMipLevels;
...
void VulkanApplication::ProcessInput()
{
    ...
    // Enable/disable shadow map LOD
    if (window_->IsKeyPressed(GLFW_KEY_0)) {
        isShadowMapLodEnabled_ = false; // Disable shadow map LOD
    }
    if (window_->IsKeyPressed(GLFW_KEY_1)) {
        isShadowMapLodEnabled_ = true;  // Enable shadow map LOD
    }
}
} // namespace examples::real_time_shadows::shadow_resource_management::mipmapped_shadow_maps
