/**
 * @file    Main.cpp
 * @brief   In this example, the mipmaps of the generated variance shadow map are created, and the shadow map LOD
 *          selection is made according to the camera's distance from the fragment. Thus, shadow mapping is done with a
 *          small shadow map size for long distances, and simple soft shadows are provided through trilinear filtering.
...
using namespace examples::real_time_shadows::shadow_resource_management::mipmapped_shadow_maps;
