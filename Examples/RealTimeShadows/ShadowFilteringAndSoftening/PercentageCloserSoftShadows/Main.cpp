/**
 * @file    Main.cpp
 * @brief   In this example, the shadows in the scene are created using Percentage Closer Soft Shadows (PCSS) with
 *          using Poisson-disk filtering.
...
using namespace examples::real_time_shadows::shadow_filtering_and_softening::percentage_closer_soft_shadows;
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::CameraSpeed, 4.5f);
