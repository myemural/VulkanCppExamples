...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto satComputeShaderAsset = assetManager_->Load<ShaderAsset>(kSatComputeShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {{.name = kSceneVertexShaderKey, .asset = assetManager_->Get(sceneVertexShaderAsset)},
                    ...
                    {.name = kSatComputeShaderKey, .asset = assetManager_->Get(satComputeShaderAsset)},
    ...
    resourceCreateInfo.images = {
        ...
        ImageResourceCreateInfo{
            .name = kShadowMapSatImage,
            .memProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            .format = VK_FORMAT_R32G32_SFLOAT,
            .dimensions = {SHADOW_MAP_SIZE, SHADOW_MAP_SIZE, 1},
            .usageFlags = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            .views = {ImageViewCreateInfo{.viewName = kShadowMapSatImageView, .format = VK_FORMAT_R32G32_SFLOAT}}},
        ImageResourceCreateInfo{
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        .maxSets = 5 + combinedImageSamplerCount + 2,
        ...
        .descriptorSets = {{.name = kMainDescSet, .layoutName = kMainDescSetLayout},
                           {.name = kSatComputeDescSet, .layoutName = kSatComputeDescSetLayout},
    ...
    std::vector<VkDescriptorImageInfo> shadowMapSatStorageImageInfos;
    shadowMapSatStorageImageInfos.emplace_back(
            VK_NULL_HANDLE, resources_->GetImageView(kShadowMapSatImage, kShadowMapSatImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_GENERAL);

    std::vector<VkDescriptorImageInfo> shadowMapSatImageInfos;
    shadowMapSatImageInfos.emplace_back(
            resources_->GetSampler(kShadowSampler)->GetHandle(),
            resources_->GetImageView(kShadowMapSatImage, kShadowMapSatImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    shadowMapTextureUpdateRequest.images = shadowMapSatImageInfos;
    shadowMapTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest satCalculationInputImageUpdateRequest;
    satCalculationInputImageUpdateRequest.descriptorSetName = kSatComputeDescSet;
    satCalculationInputImageUpdateRequest.bindingIndex = 0;
    satCalculationInputImageUpdateRequest.images = shadowMapImageInfos;
    satCalculationInputImageUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest satOutputImageUpdateRequest;
    satOutputImageUpdateRequest.descriptorSetName = kSatComputeDescSet;
    satOutputImageUpdateRequest.bindingIndex = 1;
    satOutputImageUpdateRequest.images = shadowMapSatStorageImageInfos;
    satOutputImageUpdateRequest.type = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    ...
    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        ...
        .imageWriteRequests = {textureUpdateRequest, shadowMapTextureUpdateRequest,
                               satCalculationInputImageUpdateRequest, satOutputImageUpdateRequest}};
...
void VulkanApplication::CreatePipelines()
{
    ...
    VkPushConstantRange satCalculationPushConstant;
    satCalculationPushConstant.offset = 0;
    satCalculationPushConstant.size = sizeof(SatCalculationPushConstants);
    satCalculationPushConstant.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

    satComputationPipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kSatComputeDescSetLayout)}, {satCalculationPushConstant});

    if (!satComputationPipelineLayout_) {
        throw std::runtime_error("Failed to create pipeline layout (for SAT computation)!");
    }

    satComputationPipeline_ = device_->CreateComputePipeline(satComputationPipelineLayout_, [&](auto& builder) {
        builder.SetShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kSatComputeShaderKey)->GetHandle();
        });
    });

    if (!satComputationPipeline_) {
        throw std::runtime_error("Failed to create compute pipeline (for SAT computation)!");
    }
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Barrier: Change image layout from VK_IMAGE_LAYOUT_UNDEFINED to VK_IMAGE_LAYOUT_GENERAL
    {
        const auto storageImage = resources_->GetImage(kShadowMapSatImage);
    ...
    // Compute Pass: Calculate horizontal Summed Area Table (SAT)
    {
        currentCmdBuffer->BindPipeline(satComputationPipeline_, VK_PIPELINE_BIND_POINT_COMPUTE);
        const std::vector descSets{resources_->GetDescriptorSet(kSatComputeDescSet)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_COMPUTE, satComputationPipelineLayout_, 0,
                                             descSets);

        SatCalculationPushConstants satPushConstant{};
        satPushConstant.pass = 0;
        satPushConstant.shadowMapSize = SHADOW_MAP_SIZE;
        currentCmdBuffer->PushConstants(satComputationPipelineLayout_, VK_SHADER_STAGE_COMPUTE_BIT, 0,
                                        sizeof(satPushConstant), &satPushConstant);

        const auto batchSizeX = CeilDiv(SHADOW_MAP_SIZE, 64);
        currentCmdBuffer->Dispatch(batchSizeX, 1, 1);
    }

    // Compute Pass: Calculate vertical Summed Area Table (SAT)
    {
        currentCmdBuffer->BindPipeline(satComputationPipeline_, VK_PIPELINE_BIND_POINT_COMPUTE);
        const std::vector descSets{resources_->GetDescriptorSet(kSatComputeDescSet)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_COMPUTE, satComputationPipelineLayout_, 0,
    ...
    // Barrier: Change image layout from VK_IMAGE_LAYOUT_GENERAL to VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
    {
        const auto storageImage = resources_->GetImage(kShadowMapSatImage);
