 ...
 *          Gaussian blur filtering in separate pass.
...
using namespace examples::real_time_shadows::shadow_filtering_and_softening::exponential_variance_with_gaussian;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterParam<float>(AppSettings::EsmExponent);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::EsmExponent, 40.0f);
