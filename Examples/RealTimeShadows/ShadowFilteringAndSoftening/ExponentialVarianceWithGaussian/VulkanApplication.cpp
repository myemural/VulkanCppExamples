...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto gaussianBlurComputeShaderAsset = assetManager_->Load<ShaderAsset>(kGaussianBlurComputeShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {
            ...
            {.name = kGaussianBlurComputeShaderKey, .asset = assetManager_->Get(gaussianBlurComputeShaderAsset)},
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        ...
        .layouts = {{.name = kMainDescSetLayout,
                     .bindings = {{0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT, nullptr},
                                  ...
                                  {4, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, VK_SHADER_STAGE_FRAGMENT_BIT,
                                   nullptr}}},
                    {.name = kGaussianBlurDescSetLayout,
        ...
        .descriptorSets = {{.name = kMainDescSet, .layoutName = kMainDescSetLayout},
                           {.name = kGaussianBlurDescSet, .layoutName = kGaussianBlurDescSetLayout},
    ...
    std::vector<VkDescriptorImageInfo> blurredShadowMapStorageImageInfos;
    blurredShadowMapStorageImageInfos.emplace_back(
            VK_NULL_HANDLE, resources_->GetImageView(kBlurredShadowMapImage, kBlurredShadowMapImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_GENERAL);

    std::vector<VkDescriptorImageInfo> blurredShadowMapImageInfos;
    blurredShadowMapImageInfos.emplace_back(
            resources_->GetSampler(kShadowSampler)->GetHandle(),
            resources_->GetImageView(kBlurredShadowMapImage, kBlurredShadowMapImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    shadowMapTextureUpdateRequest.images = blurredShadowMapImageInfos;
    shadowMapTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest gaussianBlurInputTextureUpdateRequest;
    gaussianBlurInputTextureUpdateRequest.descriptorSetName = kGaussianBlurDescSet;
    gaussianBlurInputTextureUpdateRequest.bindingIndex = 0;
    gaussianBlurInputTextureUpdateRequest.images = shadowMapImageInfos;
    gaussianBlurInputTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest gaussianBlurOutputTextureUpdateRequest;
    gaussianBlurOutputTextureUpdateRequest.descriptorSetName = kGaussianBlurDescSet;
    gaussianBlurOutputTextureUpdateRequest.bindingIndex = 1;
    gaussianBlurOutputTextureUpdateRequest.images = blurredShadowMapStorageImageInfos;
    gaussianBlurOutputTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    ...
    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        ...
        .imageWriteRequests = {textureUpdateRequest, shadowMapTextureUpdateRequest,
                               gaussianBlurInputTextureUpdateRequest, gaussianBlurOutputTextureUpdateRequest}};
...
void VulkanApplication::CreatePipelines()
{
    ...
    gaussianBlurPipelineLayout_ =
            device_->CreatePipelineLayout({resources_->GetDescriptorLayout(kGaussianBlurDescSetLayout)});

    if (!gaussianBlurPipelineLayout_) {
        throw std::runtime_error("Failed to create pipeline layout (for gaussian blur compute)!");
    }


    gaussianBlurPipeline_ = device_->CreateComputePipeline(gaussianBlurPipelineLayout_, [&](auto& builder) {
        builder.SetShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kGaussianBlurComputeShaderKey)->GetHandle();
        });
    });

    if (!gaussianBlurPipeline_) {
        throw std::runtime_error("Failed to create compute pipeline (for gaussian blur compute)!");
    }
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    {
        const auto storageImage = resources_->GetImage(kBlurredShadowMapImage);
    ...
    // Compute Pass: Applying gaussian blur
    {
        currentCmdBuffer->BindPipeline(gaussianBlurPipeline_, VK_PIPELINE_BIND_POINT_COMPUTE);
