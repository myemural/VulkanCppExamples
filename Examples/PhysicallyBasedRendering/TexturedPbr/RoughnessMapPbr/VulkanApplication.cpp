...
void VulkanApplication::BuildScene()
{
    ...
    const auto woodParticleAlbedoTextureAsset = assetManager_->Load<TextureAsset>(kWoodParticleAlbedoTexturePath);
    const auto woodParticleAlbedoTextureId = sceneImageStorage.StoreTexture(
            kWoodParticleAlbedoTexture, kMainSampler, assetManager_->Get(woodParticleAlbedoTextureAsset));
    const auto woodParticleRoughnessTextureAsset = assetManager_->Load<TextureAsset>(kWoodParticleRoughnessTexturePath);
    const auto woodParticleRoughnessTextureId = sceneImageStorage.StoreTexture(
            kWoodParticleRoughnessTexture, kMainSampler, assetManager_->Get(woodParticleRoughnessTextureAsset),
            VK_FORMAT_R8G8B8A8_UNORM);
    ...
    Material woodParticleMaterial;
    woodParticleMaterial.metallic = 0.0f;
    woodParticleMaterial.albedoMap = woodParticleAlbedoTextureId;
    woodParticleMaterial.roughnessMap = woodParticleRoughnessTextureId;
    ...
    const auto rootObject = SceneObjectBuilder(*scene_, kRootObject)
                                    ...
                                    .AddChild(SceneObjectBuilder(*scene_, kCubeObject1)
                                                      ...
                                                      .WithMaterial(woodParticleMaterial)
...
{
    // Change the light intensity between 1.0 and 8.0 constantly
    const auto currentTime = static_cast<float>(GetCurrentTime());
    constexpr float minIntensity = 1.0f;
    constexpr float maxIntensity = 8.0f;
