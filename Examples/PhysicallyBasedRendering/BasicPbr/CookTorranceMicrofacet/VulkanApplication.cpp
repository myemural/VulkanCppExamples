...
void VulkanApplication::BuildScene()
{
    ...
    defaultMaterial.albedoColor = glm::vec4{0.0f, 1.0f, 0.0f, 1.0f};
    ...
    for (auto i = 0U; i <= 5U; ++i) {
        ...
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, kSphereObject + rowStr)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(sphereMaterial)
                                           .WithPosition(glm::vec3{xShift, 0.0f, 0.0f}));
    }
