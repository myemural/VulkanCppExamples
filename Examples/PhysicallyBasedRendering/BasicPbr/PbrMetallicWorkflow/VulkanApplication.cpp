...
VulkanApplication::VulkanApplication(ParameterServer&& params) : ApplicationBasicPbr(std::move(params)) {}

bool VulkanApplication::Init()
{
    if (!ApplicationBasicPbr::Init()) {
...
void VulkanApplication::BuildScene()
{
    ...
    defaultMaterial.albedoColor = glm::vec4{1.0f, 0.0f, 0.0f, 1.0f};
    ...
    for (auto i = 0U; i <= 5U; ++i) {
        for (auto j = 0U; j <= 5U; ++j) {
            const std::string rowColStr = std::to_string(i) + "_" + std::to_string(j);
            const auto xShift = -static_cast<float>(i * 2 - 1) + 5.0f;
            const auto yShift = -static_cast<float>(j * 2 - 1) + 5.0f;
            Material sphereMaterial = defaultMaterial;
            sphereMaterial.roughness = 0.0f + static_cast<float>(i) * 0.2f;
            sphereMaterial.metallic = 0.0f + static_cast<float>(j) * 0.2f;

            rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, kSphereObject + rowColStr)
                                               .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                               .WithMaterial(sphereMaterial)
                                               .WithPosition(glm::vec3{xShift, yShift, 0.0f}));
        }
    }
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    const float angularSpeed = 0.4f * currentTime;
    constexpr float radius = 9.0f;
    const float x = radius * cos(angularSpeed);
    const float z = radius * sin(angularSpeed);
    scene_->FindObjectByName(kLightObject)->SetPosition(glm::vec3(x, 5.0f, z));
