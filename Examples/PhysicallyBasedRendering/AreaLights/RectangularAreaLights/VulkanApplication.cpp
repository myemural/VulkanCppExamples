...
void VulkanApplication::BuildScene()
{
    ...
    floorMaterial.albedoColor = glm::vec4(0.75f, 0.75f, 0.75f, 1.0f);
    floorMaterial.roughness = GetParamFloat(AppSettings::FloorRoughness);
    floorMaterial.metallic = GetParamFloat(AppSettings::FloorMetallic);
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    const auto planeCorners = ComputePlaneCornersInWorldSpace(lightWorldMat);
    ...
    lightUbo.lightRectCornerPos = planeCorners;
