...
void VulkanApplication::BuildScene()
{
    ...
    const auto damagedHelmetModelHandle = assetManager_->Load<GltfModelAsset>(kDamagedHelmetModelPath);
    const auto damagedHelmetModel = std::make_unique<GltfModelAsset>(assetManager_->Get(damagedHelmetModelHandle));

    GltfToSceneObjectConverter converter{*scene_, kMainSampler};
    auto damagedHelmetModelSceneObjectBuilder =
            converter.ConvertToBuilder(kDamagedHelmetModelName, *damagedHelmetModel);

    const auto rootObject =
            SceneObjectBuilder(*scene_, kRootObject)
                    .WithPosition(glm::vec3{0.0f, 0.0f, 0.0f})
                    .AddChild(damagedHelmetModelSceneObjectBuilder.WithPosition(glm::vec3{0.0f, 0.0f, 0.0f})
