/**
 * @file    Main.cpp
 * @brief   In this example, the PBR material system is first implemented using deferred shading logic, and then the
 *          Screen-Space Reflection (SSR) implementation is demonstrated in a separate pass.
...
using namespace examples::physically_based_rendering::ibl_and_reflections::screen_space_reflections;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterParam<float>(AppSettings::SsrMaxDistance);
    schema.RegisterParam<float>(AppSettings::SsrThickness);
    schema.RegisterParam<std::int32_t>(AppSettings::SsrMaxSteps);
    schema.RegisterParam<std::int32_t>(AppSettings::SsrBinarySearchSteps);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::SsrMaxDistance, 10.0f);
        params.Set(AppSettings::SsrThickness, 0.4f);
        params.Set<std::int32_t>(AppSettings::SsrMaxSteps, 128);
        params.Set<std::int32_t>(AppSettings::SsrBinarySearchSteps, 16);
