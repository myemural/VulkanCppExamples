...
namespace examples::physically_based_rendering::disney_principled_brdf::disney_brdf_anisotropy
{
...
void VulkanApplication::BuildScene()
{
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        ...
        Material anisotropicTestMaterial;
        anisotropicTestMaterial.albedoColor = glm::vec4{0.5f, 0.5f, 0.5f, 1.0f};
        anisotropicTestMaterial.roughness = 0.2f;
        anisotropicTestMaterial.metallic = 1.0f;
        anisotropicTestMaterial.anisotropic = kValueIncrementPerObject * static_cast<float>(i);
        ...
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           ...
                                           .WithMaterial(anisotropicTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }

    // Anisotropic roughness test
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        const auto currentPosX = objectStartPosX + static_cast<float>(i) * objectDistanceBetween;
        const auto currentObjectPosition = glm::vec3(currentPosX, -1.5f, 0.0f);

        Material anisotropicRoughnessTestMaterial;
        anisotropicRoughnessTestMaterial.albedoColor = glm::vec4{0.5f, 0.5f, 0.5f, 1.0f};
        anisotropicRoughnessTestMaterial.roughness = kValueIncrementPerObject * static_cast<float>(i);
        anisotropicRoughnessTestMaterial.metallic = 0.8f;
        anisotropicRoughnessTestMaterial.anisotropic = 1.0f;

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(anisotropicRoughnessTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }
