...
VulkanApplication::VulkanApplication(ParameterServer&& params) : ApplicationDisneyPrincipledBrdf(std::move(params)) {}

bool VulkanApplication::Init()
{
    if (!ApplicationDisneyPrincipledBrdf::Init()) {
...
{
    // Poll events
    ApplicationDisneyPrincipledBrdf::PreUpdate();
...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto brdfLutGeneratorComputeShaderAsset =
            assetManager_->Load<ShaderAsset>(kBrdfLutGeneratorComputeShaderFile);
    const auto prefilteredCubemapComputeShaderAsset =
            assetManager_->Load<ShaderAsset>(kPrefilteredCubemapComputeShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {
            ...
            {.name = kBrdfLutGeneratorComputeShaderKey,
             .asset = assetManager_->Get(brdfLutGeneratorComputeShaderAsset)},
            {.name = kPrefilteredCubemapComputeShaderKey,
             .asset = assetManager_->Get(prefilteredCubemapComputeShaderAsset)}}};
    ...
    resourceCreateInfo.images =
            {ImageResourceCreateInfo{
             ...
             ImageResourceCreateInfo{
                 ...
                 .dimensions = {kBrdfLutImageSize, kBrdfLutImageSize, 1},
                 .usageFlags = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                 .views = {ImageViewCreateInfo{.viewName = kBrdfLutImageView, .format = VK_FORMAT_R16G16_SFLOAT}}},
             ImageResourceCreateInfo{
                 .name = kPrefilterCubemapImage,
                 .memProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                 .createFlags = VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT,
                 .format = VK_FORMAT_R32G32B32A32_SFLOAT,
                 .dimensions = {kPrefilterCubemapSize, kPrefilterCubemapSize, 1},
                 .mipLevels = kPrefilterRoughnessMaxMipCount,
                 .arrayLayers = 6,
                 .usageFlags = VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                 .views = {ImageViewCreateInfo{
                     .viewName = kPrefilterCubemapImageView,
                     .viewType = VK_IMAGE_VIEW_TYPE_CUBE,
                     .format = VK_FORMAT_R32G32B32A32_SFLOAT,
                     .subresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, kPrefilterRoughnessMaxMipCount, 0, 6}}}}};
...
void VulkanApplication::BuildScene()
{
    ...
    camera_ = std::make_shared<PerspectiveCamera>(glm::vec3(0.0f, 0.0f, 18.0f), aspectRatio);
    ...
    [[maybe_unused]] const auto hdrEnvironmentTextureId = sceneImageStorage.StoreTexture(
            kHdrEnvironmentTexture, kMainSampler, assetManager_->Get(hdrEnvironmentTextureAsset));

    auto rootObjectBuilder = SceneObjectBuilder(*scene_, kRootObject).WithPosition(glm::vec3{0.0f, 0.0f, 0.0f});

    constexpr auto objectStartPosX = -1.5f * static_cast<float>(kObjectCountPerRow - 1);
    constexpr auto objectDistanceBetween = 3.0f;
    std::uint32_t globalObjectCounter = 0;

    // Roughness test
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        const auto currentPosX = objectStartPosX + static_cast<float>(i) * objectDistanceBetween;
        const auto currentObjectPosition = glm::vec3(currentPosX, 6.0f, 0.0f);

        Material roughnessTestMaterial;
        roughnessTestMaterial.albedoColor = glm::vec4{1.0f, 0.0f, 0.0f, 1.0f};
        roughnessTestMaterial.roughness = kValueIncrementPerObject * static_cast<float>(i);
        roughnessTestMaterial.metallic = 0.0f;

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(roughnessTestMaterial)
                                           .WithPosition(currentObjectPosition)
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        const auto currentPosX = objectStartPosX + static_cast<float>(i) * objectDistanceBetween;
        const auto currentObjectPosition = glm::vec3(currentPosX, 3.0f, 0.0f);

        Material metallicTestMaterial;
        metallicTestMaterial.albedoColor = glm::vec4{0.0f, 1.0f, 0.0f, 1.0f};
        metallicTestMaterial.roughness = 0.3f;
        metallicTestMaterial.metallic = kValueIncrementPerObject * static_cast<float>(i);

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(metallicTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        const auto currentPosX = objectStartPosX + static_cast<float>(i) * objectDistanceBetween;
        const auto currentObjectPosition = glm::vec3(currentPosX, 0.0f, 0.0f);

        Material specularTestMaterial;
        specularTestMaterial.albedoColor = glm::vec4{1.0f, 0.0f, 0.0f, 1.0f};
        specularTestMaterial.roughness = 0.2f;
        specularTestMaterial.metallic = 0.0f;
        specularTestMaterial.specularStrength = kValueIncrementPerObject * static_cast<float>(i);
        specularTestMaterial.specularTint = 0.0f;

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(specularTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        ...
        Material specularTintTestMaterial;
        specularTintTestMaterial.albedoColor = glm::vec4{0.0f, 1.0f, 0.0f, 1.0f};
        specularTintTestMaterial.roughness = 0.2f;
        specularTintTestMaterial.metallic = 0.0f;
        specularTintTestMaterial.specularStrength = 1.0;
        specularTintTestMaterial.specularTint = kValueIncrementPerObject * static_cast<float>(i);

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(specularTintTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        ...
        Material subsurfaceTestMaterial;
        subsurfaceTestMaterial.albedoColor = glm::vec4{0.8f, 0.55f, 0.45f, 1.0f};
        subsurfaceTestMaterial.roughness = 0.8f;
        subsurfaceTestMaterial.metallic = 0.0f;
        subsurfaceTestMaterial.subsurface = kValueIncrementPerObject * static_cast<float>(i);

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(subsurfaceTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        .maxSets = 4 + combinedImageSamplerCount + 4 + kPrefilterRoughnessMaxMipCount,
...
void VulkanApplication::CreatePipelines()
{
    ...
    pipelineLayoutBrdfLut_ = device_->CreatePipelineLayout({resources_->GetDescriptorLayout(kBrdfLutDescSetLayout)});

    if (!pipelineLayoutBrdfLut_) {
        throw std::runtime_error("Failed to create compute pipeline layout (for BRDF LUT)!");
    }

    brdfLutPipeline_ = device_->CreateComputePipeline(pipelineLayoutBrdfLut_, [&](auto& builder) {
        builder.SetShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kBrdfLutGeneratorComputeShaderKey)->GetHandle();
        });
    });

    if (!brdfLutPipeline_) {
        throw std::runtime_error("Failed to create compute pipeline (for BRDF LUT)!");
    }

    VkPushConstantRange prefilterMapPushConstant;
    prefilterMapPushConstant.offset = 0;
    prefilterMapPushConstant.size = sizeof(PrefilterMapPushConstants);
    prefilterMapPushConstant.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;

    pipelineLayoutPrefilteredCubemap_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kPrefilterMapDescSetLayout)}, {prefilterMapPushConstant});

    if (!pipelineLayoutPrefilteredCubemap_) {
        throw std::runtime_error("Failed to create compute pipeline layout (for prefiltered cubemap)!");
    }

    prefilteredCubemapPipeline_ = device_->CreateComputePipeline(pipelineLayoutPrefilteredCubemap_, [&](auto& builder) {
        builder.SetShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
            shaderStageCreateInfo.module =
                    resources_->GetShaderModule(kPrefilteredCubemapComputeShaderKey)->GetHandle();
        });
    });

    if (!prefilteredCubemapPipeline_) {
        throw std::runtime_error("Failed to create compute pipeline (for prefiltered cubemap)!");
    }
}
...
void VulkanApplication::PrecalculateCubemapImages()
{
    ...
    // Compute phase
    {
        ...
        const auto storageImage = resources_->GetImage(kBrdfLutImage);
        ...
        currentCmdBuffer->BindPipeline(brdfLutPipeline_, VK_PIPELINE_BIND_POINT_COMPUTE);
        const std::vector descSets{resources_->GetDescriptorSet(kBrdfLutDescSet)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayoutBrdfLut_, 0, descSets);

        const auto groupCountX = CeilDiv(kBrdfLutImageSize, kLocalThreadCountX);
        const auto groupCountY = CeilDiv(kBrdfLutImageSize, kLocalThreadCountY);
    ...
    {
        const auto prefilterImage = resources_->GetImage(kPrefilterCubemapImage);

        // Change image layout from VK_IMAGE_LAYOUT_UNDEFINED to VK_IMAGE_LAYOUT_GENERAL
        auto barrierUndefinedToGeneral = prefilterImage->CreateImageMemoryBarrier(
                0, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL,
                VkImageSubresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, kPrefilterRoughnessMaxMipCount, 0, 6});
        currentCmdBuffer->PipelineBarrier(VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                          {barrierUndefinedToGeneral});

        currentCmdBuffer->BindPipeline(prefilteredCubemapPipeline_, VK_PIPELINE_BIND_POINT_COMPUTE);
        for (uint32_t mip = 0; mip < kPrefilterRoughnessMaxMipCount; mip++) {
            std::vector descSets{resources_->GetDescriptorSet(kPrefilterMapDescSet + std::to_string(mip))};
            currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayoutPrefilteredCubemap_, 0,
                                                 descSets);

            const uint32_t mipSize = kPrefilterCubemapSize >> mip;
            PrefilterMapPushConstants pushConstant{};
            pushConstant.roughness = static_cast<float>(mip) / static_cast<float>(kPrefilterRoughnessMaxMipCount - 1);
            pushConstant.mipLevel = mip;
            pushConstant.mipSize = mipSize;
            currentCmdBuffer->PushConstants(pipelineLayoutPrefilteredCubemap_, VK_SHADER_STAGE_COMPUTE_BIT, 0,
                                            sizeof(pushConstant), &pushConstant);

            const uint32_t groupX = CeilDiv(mipSize, kLocalThreadCountX);
            const uint32_t groupY = CeilDiv(mipSize, kLocalThreadCountY);
            constexpr uint32_t groupZ = 6; // Array size

            currentCmdBuffer->Dispatch(groupX, groupY, groupZ);
        }

        // Change image layout from VK_IMAGE_LAYOUT_GENERAL to VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
        auto barrierGeneralToShaderRead = prefilterImage->CreateImageMemoryBarrier(
                VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                VkImageSubresourceRange{VK_IMAGE_ASPECT_COLOR_BIT, 0, kPrefilterRoughnessMaxMipCount, 0, 6});
        currentCmdBuffer->PipelineBarrier(VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    scene_->Traverse([&](const SceneObject& sceneObject) {
        if (sceneObject.HasRenderable() && sceneObject.GetTag() != kSkyboxObjectGroup) {
            ...
            scenePushConstants.prefilterRoughnessMaxMipCount = kPrefilterRoughnessMaxMipCount;
            scenePushConstants.isIblEnabled = isIblEnabled_ ? 1U : 0U;
...
void VulkanApplication::ProcessInput()
{
    ...
    // Enable/disable IBL for debug purposes
