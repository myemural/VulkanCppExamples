...
namespace examples::physically_based_rendering::disney_principled_brdf::disney_brdf_clear_coat
{
...
void VulkanApplication::BuildScene()
{
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        ...
        const auto currentObjectPosition = glm::vec3(currentPosX, 1.5f, 0.0f);

        Material clearcoatTestMaterial;
        clearcoatTestMaterial.albedoColor = glm::vec4{0.15f, 0.15f, 0.15f, 1.0f};
        clearcoatTestMaterial.roughness = 0.8f;
        clearcoatTestMaterial.metallic = 0.0f;
        clearcoatTestMaterial.clearcoat = kValueIncrementPerObject * static_cast<float>(i);
        clearcoatTestMaterial.clearcoatGloss = 1.0f;
        ...
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           ...
                                           .WithMaterial(clearcoatTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }

    // Clear coat glossiness test
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        const auto currentPosX = objectStartPosX + static_cast<float>(i) * objectDistanceBetween;
        const auto currentObjectPosition = glm::vec3(currentPosX, -1.5f, 0.0f);

        Material clearcoatGlossTestMaterial;
        clearcoatGlossTestMaterial.albedoColor = glm::vec4{0.15f, 0.15f, 0.15f, 1.0f};
        clearcoatGlossTestMaterial.roughness = 0.8f;
        clearcoatGlossTestMaterial.metallic = 0.0f;
        clearcoatGlossTestMaterial.clearcoat = 1.0f;
        clearcoatGlossTestMaterial.clearcoatGloss = kValueIncrementPerObject * static_cast<float>(i);

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(clearcoatGlossTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }
