...
void VulkanApplication::BuildScene()
{
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        ...
        Material sheenTestMaterial;
        sheenTestMaterial.albedoColor = glm::vec4{0.9f, 0.0f, 0.0f, 1.0f};
        sheenTestMaterial.roughness = 0.8f;
        sheenTestMaterial.metallic = 0.0f;
        sheenTestMaterial.sheen = kValueIncrementPerObject * static_cast<float>(i);
        sheenTestMaterial.sheenTint = 0.0f;
        ...
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           ...
                                           .WithMaterial(sheenTestMaterial)
    ...
    for (auto i = 0U; i < kObjectCountPerRow; ++i) {
        ...
        Material sheenTintTestMaterial;
        sheenTintTestMaterial.albedoColor = glm::vec4{0.9f, 0.0f, 0.0f, 1.0f};
        sheenTintTestMaterial.roughness = 0.8f;
        sheenTintTestMaterial.metallic = 0.0f;
        sheenTintTestMaterial.sheen = 1.0f;
        sheenTintTestMaterial.sheenTint = kValueIncrementPerObject * static_cast<float>(i);

        const std::string currentObjectName = kSphereObject + std::to_string(globalObjectCounter++);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, currentObjectName)
                                           .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                           .WithMaterial(sheenTintTestMaterial)
                                           .WithPosition(currentObjectPosition)
                                           .WithScale(glm::vec3{2.0f}));
    }
