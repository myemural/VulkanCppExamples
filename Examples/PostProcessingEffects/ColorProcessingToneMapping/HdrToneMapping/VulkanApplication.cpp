...
void VulkanApplication::CreatePipelines()
{
    ...
    toneMappingPushConstant.offset = 0;
    toneMappingPushConstant.size = sizeof(ToneMappingPushConstants);
    toneMappingPushConstant.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    postProcessingPipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kPostProcessingDescSetLayout)}, {toneMappingPushConstant});
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Post-Processing Pass
    {
        ...
        toneMappingPushConstants.exposure = exposure_;
