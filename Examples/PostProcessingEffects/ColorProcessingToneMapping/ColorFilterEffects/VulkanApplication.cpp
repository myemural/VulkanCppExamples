...
namespace examples::post_processing_effects::color_processing_tone_mapping::color_filter_effects
{
...
void VulkanApplication::CreatePipelines()
{
    ...
    VkPushConstantRange colorFilterPushConstant;
    colorFilterPushConstant.offset = 0;
    colorFilterPushConstant.size = sizeof(ColorFilterPushConstants);
    colorFilterPushConstant.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    postProcessingPipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kPostProcessingDescSetLayout)}, {colorFilterPushConstant});
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Post-Processing Pass
    {
        ...
        ColorFilterPushConstants colorFilterPushConstants{};
        colorFilterPushConstants.duoToneDarkColor = glm::vec4(kDuoToneDarkColor, 1.0f);
        colorFilterPushConstants.duoToneBrightColor = glm::vec4(kDuoToneBrightColor, 1.0f);
        colorFilterPushConstants.filterMode = static_cast<std::uint32_t>(filterMode_);
        currentCmdBuffer->PushConstants(postProcessingPipelineLayout_, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                        sizeof(colorFilterPushConstants), &colorFilterPushConstants);
...
void VulkanApplication::ProcessInput()
{
    ...
    // Set filter mode via num keys
    if (window_->IsKeyPressed(GLFW_KEY_0)) {
        filterMode_ = FilterMode::OFF;
    }
    if (window_->IsKeyPressed(GLFW_KEY_1)) {
        filterMode_ = FilterMode::GRAYSCALE;
    }
    if (window_->IsKeyPressed(GLFW_KEY_2)) {
        filterMode_ = FilterMode::INVERT_COLORS;
    }
    if (window_->IsKeyPressed(GLFW_KEY_3)) {
        filterMode_ = FilterMode::SEPIA;
    }
    if (window_->IsKeyPressed(GLFW_KEY_4)) {
        filterMode_ = FilterMode::DUO_TONE;
    }
    if (window_->IsKeyPressed(GLFW_KEY_5)) {
        filterMode_ = FilterMode::CHANNEL_SWAP;
    }
}
} // namespace examples::post_processing_effects::color_processing_tone_mapping::color_filter_effects
