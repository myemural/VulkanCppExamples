...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        ...
        .poolSizes = {{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 3},
                      ...
                      {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, combinedImageSamplerCount + 5}},
        .layouts =
                {
    ...
    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        ...
        .imageWriteRequests = {textureUpdateRequest, positionUpdateRequest, albedoUpdateRequest, normalUpdateRequest,
                               roughnessMetallicUpdateRequest, lightingOutputUpdateRequest}};
...
void VulkanApplication::InitInputSystem()
{
    ...
    window_->OnKey([&](const KeyEvent& event) {
        ...
        constexpr float kLiftStep = 0.1f;
        constexpr float kGammaStep = 0.1f;
        constexpr float kGainStep = 0.1f;
        ...
        switch (event.key) {
            case GLFW_KEY_SPACE:
                ...
                break;
            case GLFW_KEY_0:
                currentLift = kNeutralPresetLift;
                currentGamma = kNeutralPresetGamma;
                currentGain = kNeutralPresetGain;
                std::cout << "Current preset: Neutral" << std::endl;
                break;
            case GLFW_KEY_1:
                currentLift = kTealOrangePresetLift;
                currentGamma = kTealOrangePresetGamma;
                currentGain = kTealOrangePresetGain;
                std::cout << "Current preset: Teal & Orange" << std::endl;
                break;
            case GLFW_KEY_2:
                currentLift = kGoldenHourPresetLift;
                currentGamma = kGoldenHourPresetGamma;
                currentGain = kGoldenHourPresetGain;
                std::cout << "Current preset: Golden Hour" << std::endl;
                break;
            case GLFW_KEY_3:
                currentLift = kColdBleakPresetLift;
                currentGamma = kColdBleakPresetGamma;
                currentGain = kColdBleakPresetGain;
                std::cout << "Current preset: Cold Bleak" << std::endl;
                break;
            case GLFW_KEY_4:
                currentLift = kStickyGreenPresetLift;
                currentGamma = kStickyGreenPresetGamma;
                currentGain = kStickyGreenPresetGain;
                std::cout << "Current preset: Sticky Green" << std::endl;
                break;
            case GLFW_KEY_Q:
            ...
            case GLFW_KEY_E:
                ...
                break;
            case GLFW_KEY_R:
                liftOffset_ = std::min(liftOffset_ + kLiftStep, 1.0f);
                std::cout << "Current lift offset: " << liftOffset_ << std::endl;
                break;
            case GLFW_KEY_F:
                liftOffset_ = std::max(liftOffset_ - kLiftStep, -1.0f);
                std::cout << "Current lift offset: " << liftOffset_ << std::endl;
                break;
            case GLFW_KEY_T:
                gammaOffset_ = std::min(gammaOffset_ + kGammaStep, 1.0f);
                std::cout << "Current gamma offset: " << gammaOffset_ << std::endl;
                break;
            case GLFW_KEY_G:
                gammaOffset_ = std::max(gammaOffset_ - kGammaStep, -1.0f);
                std::cout << "Current gamma offset: " << gammaOffset_ << std::endl;
                break;
            case GLFW_KEY_Y:
                gainOffset_ = std::min(gainOffset_ + kGainStep, 1.0f);
                std::cout << "Current gain offset: " << gainOffset_ << std::endl;
                break;
            case GLFW_KEY_H:
                gainOffset_ = std::max(gainOffset_ - kGainStep, -1.0f);
                std::cout << "Current gain offset: " << gainOffset_ << std::endl;
                break;
            default:
                break;
...
void VulkanApplication::CreatePipelines()
{
    ...
    VkPushConstantRange liftGammaGainPushConstant;
    liftGammaGainPushConstant.offset = 0;
    liftGammaGainPushConstant.size = sizeof(LiftGammaGainPushConstants);
    liftGammaGainPushConstant.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    postProcessingPipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kPostProcessingDescSetLayout)}, {liftGammaGainPushConstant});
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Post-Processing Pass
    {
        ...
        LiftGammaGainPushConstants liftGammaGainPushConstants{};
        liftGammaGainPushConstants.lift = glm::vec4(currentLift, liftOffset_);
        liftGammaGainPushConstants.gamma = glm::vec4(currentGamma, gammaOffset_);
        liftGammaGainPushConstants.gain = glm::vec4(currentGain, gainOffset_);
        liftGammaGainPushConstants.exposure = exposure_;
        currentCmdBuffer->PushConstants(postProcessingPipelineLayout_, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                        sizeof(liftGammaGainPushConstants), &liftGammaGainPushConstants);
