    ...
    window_->OnKey([&](const KeyEvent& event) {
        ...
        switch (event.key) {
            case GLFW_KEY_SPACE:
                ...
                saturation_ = 1.0f;
                hueShift_ = 0.0f;
                isInDisplayEncodingApplied_ = true;
            ...
            case GLFW_KEY_J:
                ...
                break;
            case GLFW_KEY_Q:
                isInDisplayEncodingApplied_ = !isInDisplayEncodingApplied_;
                std::cout << "Currently applied encoding: " << (isInDisplayEncodingApplied_ ? "sRGB" : "Linear")
                          << std::endl;
                break;
            default:
                break;
...
void VulkanApplication::CreatePipelines()
{
    ...
    VkPushConstantRange colorCorrectionPushConstant;
    colorCorrectionPushConstant.offset = 0;
    colorCorrectionPushConstant.size = sizeof(ColorCorrectionPushConstants);
    colorCorrectionPushConstant.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    postProcessingPipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kPostProcessingDescSetLayout)}, {colorCorrectionPushConstant});
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Post-Processing Pass
    {
        ...
        ColorCorrectionPushConstants colorCorrectionPushConstants{};
        colorCorrectionPushConstants.brightness = brightness_;
        colorCorrectionPushConstants.contrast = contrast_;
        colorCorrectionPushConstants.saturation = saturation_;
        colorCorrectionPushConstants.hueShift = hueShift_;
        colorCorrectionPushConstants.applyInDisplayEncoding = isInDisplayEncodingApplied_ ? 1U : 0U;
        currentCmdBuffer->PushConstants(postProcessingPipelineLayout_, VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                        sizeof(colorCorrectionPushConstants), &colorCorrectionPushConstants);
