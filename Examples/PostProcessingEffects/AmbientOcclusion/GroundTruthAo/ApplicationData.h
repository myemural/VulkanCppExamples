...
// Constants
inline constexpr auto kGtaoSliceCount = 4U;
inline constexpr auto kGtaoStepPerSlice = 8U;
...
inline constexpr auto kInitialAmbientStrength = 0.55f;
inline constexpr auto kInitialGtaoRadius = 1.5f;
inline constexpr auto kInitialGtaoThickness = 0.85f;
inline constexpr auto kInitialGtaoPower = 1.5f;
...
enum class DebugMode : std::uint32_t
{
    ...
    GTAO_RAW,     // Raw Occlusion Factor Output
    GTAO_BLURRED, // Blurred Occlusion Factor Output
...
{
    glm::mat4 projection;
    float radius;    // View-space radius of the horizon search
    float thickness; // Thin occluder compensation, 1.0 assumes infinitely thick occluders
    float power;
    std::uint32_t sliceCount;
    std::uint32_t stepsPerSlice;
};

struct LightingPushConstants
{
    std::uint32_t debugMode;
    std::uint32_t gtaoEnabled;
