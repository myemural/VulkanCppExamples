...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto gtaoFragmentShaderAsset = assetManager_->Load<ShaderAsset>(kGtaoFragmentShaderFile);
    const auto gtaoBlurFragmentShaderAsset = assetManager_->Load<ShaderAsset>(kGtaoBlurFragmentShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {
            ...
            {.name = kGtaoFragmentShaderKey, .asset = assetManager_->Get(gtaoFragmentShaderAsset)},
            {.name = kGtaoBlurFragmentShaderKey, .asset = assetManager_->Get(gtaoBlurFragmentShaderAsset)},
    ...
    resourceCreateInfo.images = {
        ...
        ImageResourceCreateInfo{
            ...
            .dimensions = {currentWindowWidth_, currentWindowHeight_, 1},
            .usageFlags = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            .views = {ImageViewCreateInfo{.viewName = kGtaoImageView, .format = VK_FORMAT_R8_UNORM}}},
        ImageResourceCreateInfo{
            .name = kGtaoBlurImage,
            .memProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            .format = VK_FORMAT_R8_UNORM,
            .dimensions = {currentWindowWidth_, currentWindowHeight_, 1},
            .usageFlags = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            .views = {ImageViewCreateInfo{.viewName = kGtaoBlurImageView, .format = VK_FORMAT_R8_UNORM}}},
        ImageResourceCreateInfo{
            .name = kDepthImage,
            .memProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        ...
        .poolSizes = {{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2},
                      ...
                      {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, combinedImageSamplerCount + 8}},
        .layouts =
                {
        ...
        .descriptorSets = {{.name = kMainDescSet, .layoutName = kMainDescSetLayout},
                           {.name = kGtaoDescSet, .layoutName = kGtaoDescSetLayout},
                           {.name = kGtaoBlurDescSet, .layoutName = kGtaoBlurDescSetLayout},
    ...
    gtaoImageInfos.emplace_back(resources_->GetSampler(kGBufferSampler)->GetHandle(),
                                resources_->GetImageView(kGtaoImage, kGtaoImageView)->GetHandle(),
                                VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    std::vector<VkDescriptorImageInfo> gtaoBlurImageInfos;
    gtaoBlurImageInfos.emplace_back(resources_->GetSampler(kGBufferSampler)->GetHandle(),
                                    resources_->GetImageView(kGtaoBlurImage, kGtaoBlurImageView)->GetHandle(),
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    // GTAO pass
    ImageWriteRequest gtaoPositionUpdateRequest;
    gtaoPositionUpdateRequest.descriptorSetName = kGtaoDescSet;
    gtaoPositionUpdateRequest.bindingIndex = 0;
    gtaoPositionUpdateRequest.images = positionImageInfos;
    gtaoPositionUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest gtaoNormalUpdateRequest;
    gtaoNormalUpdateRequest.descriptorSetName = kGtaoDescSet;
    gtaoNormalUpdateRequest.bindingIndex = 1;
    gtaoNormalUpdateRequest.images = normalImageInfos;
    gtaoNormalUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    // GTAO blur pass
    ImageWriteRequest gtaoBlurInputUpdateRequest;
    gtaoBlurInputUpdateRequest.descriptorSetName = kGtaoBlurDescSet;
    gtaoBlurInputUpdateRequest.bindingIndex = 0;
    gtaoBlurInputUpdateRequest.images = gtaoImageInfos;
    gtaoBlurInputUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    ...
    ImageWriteRequest gtaoUpdateRequest;
    gtaoUpdateRequest.descriptorSetName = kLightDescSet;
    gtaoUpdateRequest.bindingIndex = 3;
    gtaoUpdateRequest.images = gtaoImageInfos;
    gtaoUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    ImageWriteRequest gtaoBlurUpdateRequest;
    gtaoBlurUpdateRequest.descriptorSetName = kLightDescSet;
    gtaoBlurUpdateRequest.bindingIndex = 4;
    gtaoBlurUpdateRequest.images = gtaoBlurImageInfos;
    gtaoBlurUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    ...
    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        ...
        .imageWriteRequests = {textureUpdateRequest, gtaoPositionUpdateRequest, gtaoNormalUpdateRequest,
                               gtaoBlurInputUpdateRequest, positionUpdateRequest, albedoUpdateRequest,
                               normalUpdateRequest, gtaoUpdateRequest, gtaoBlurUpdateRequest}};
...
void VulkanApplication::InitInputSystem()
{
    ...
    window_->OnKey([&](const KeyEvent& event) {
        ...
        constexpr auto kThicknessStep = 0.05f;
        ...
        switch (event.key) {
            ...
            case GLFW_KEY_2:
                ...
                break;
            case GLFW_KEY_3:
                debugMode_ = DebugMode::GTAO_RAW;
                std::cout << "Debug mode changed: GTAO Raw" << std::endl;
                break;
            case GLFW_KEY_4:
                debugMode_ = DebugMode::GTAO_BLURRED;
                std::cout << "Debug mode changed: GTAO Blurred" << std::endl;
                break;
            case GLFW_KEY_5:
                ...
                break;
            case GLFW_KEY_SPACE:
                isGtaoEnabled_ = !isGtaoEnabled_;
                std::cout << "GTAO enabled: " << (isGtaoEnabled_ ? "YES" : "NO") << std::endl;
                break;
            case GLFW_KEY_Z:
                gtaoRadius_ = std::max(gtaoRadius_ - kRadiusStep, 0.0f);
                std::cout << "New GTAO Radius: " << gtaoRadius_ << std::endl;
                break;
            case GLFW_KEY_X:
                gtaoRadius_ = std::min(gtaoRadius_ + kRadiusStep, 4.0f);
                std::cout << "New GTAO Radius: " << gtaoRadius_ << std::endl;
                break;
            case GLFW_KEY_C:
                gtaoThickness_ = std::max(gtaoThickness_ - kThicknessStep, 0.0f);
                std::cout << "New GTAO Thickness: " << gtaoThickness_ << std::endl;
                break;
            case GLFW_KEY_V:
                gtaoThickness_ = std::min(gtaoThickness_ + kThicknessStep, 1.0f);
                std::cout << "New GTAO Thickness: " << gtaoThickness_ << std::endl;
                break;
            case GLFW_KEY_B:
                gtaoPower_ = std::max(gtaoPower_ - kPowerStep, 0.5f);
                std::cout << "New GTAO Power: " << gtaoPower_ << std::endl;
                break;
            case GLFW_KEY_N:
                gtaoPower_ = std::min(gtaoPower_ + kPowerStep, 5.0f);
                std::cout << "New GTAO Power: " << gtaoPower_ << std::endl;
                break;
            default:
                break;
...
void VulkanApplication::CreateRenderPass()
{
    ...
    // This render pass is shared by the GTAO blur passes
    VkAttachmentReference occlusionColorAttachment = {0, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};

    gtaoRenderPass_ = device_->CreateRenderPass([&](auto& builder) {
        builder.AddAttachment([](auto& attachmentCreateInfo) {
    ...
    if (!gtaoRenderPass_) {
        throw std::runtime_error("Failed to create render pass (for GTAO)!");
    }
...
void VulkanApplication::CreatePipelines()
{
    ...
    gtaoPushConstant.offset = 0;
    gtaoPushConstant.size = sizeof(GtaoPushConstants);
    gtaoPushConstant.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

    gtaoPipelineLayout_ =
            device_->CreatePipelineLayout({resources_->GetDescriptorLayout(kGtaoDescSetLayout)}, {gtaoPushConstant});

    if (!gtaoPipelineLayout_) {
        throw std::runtime_error("Failed to create pipeline layout!");
    }

    gtaoPassPipeline_ = device_->CreateGraphicsPipeline(gtaoPipelineLayout_, gtaoRenderPass_, [&](auto& builder) {
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
        ...
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kGtaoFragmentShaderKey)->GetHandle();
        });
        builder.SetVertexInputState([&](auto& vertexInputStateCreateInfo) {
    ...
    if (!gtaoPassPipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (for GTAO)!");
    }

    gtaoBlurPipelineLayout_ = device_->CreatePipelineLayout({resources_->GetDescriptorLayout(kGtaoBlurDescSetLayout)});

    if (!gtaoBlurPipelineLayout_) {
        throw std::runtime_error("Failed to create pipeline layout!");
    }

    gtaoBlurPassPipeline_ =
            device_->CreateGraphicsPipeline(gtaoBlurPipelineLayout_, gtaoRenderPass_, [&](auto& builder) {
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
                ...
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
                    shaderStageCreateInfo.module = resources_->GetShaderModule(kGtaoBlurFragmentShaderKey)->GetHandle();
                });
                builder.SetVertexInputState([&](auto& vertexInputStateCreateInfo) {
                    vertexInputStateCreateInfo.vertexBindingDescriptionCount = 0;
                    vertexInputStateCreateInfo.vertexAttributeDescriptionCount = 0;
    ...
    if (!gtaoBlurPassPipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (for GTAO blur)!");
    }
...
void VulkanApplication::CreateFramebuffers()
{
    ...
    // GTAO framebuffer
    const auto& gtaoImageView = resources_->GetImageView(kGtaoImage, kGtaoImageView);
    gtaoFramebuffer_ = device_->CreateFramebuffer(gtaoRenderPass_, {gtaoImageView}, [&](auto& builder) {
        builder.SetDimensions(currentWindowWidth_, currentWindowHeight_);
    });

    if (!gtaoFramebuffer_) {
        throw std::runtime_error("Failed to create framebuffer (for GTAO)!");
    }

    // GTAO blur framebuffer
    const auto& gtaoBlurImageView = resources_->GetImageView(kGtaoBlurImage, kGtaoBlurImageView);
    gtaoBlurFramebuffer_ = device_->CreateFramebuffer(gtaoRenderPass_, {gtaoBlurImageView}, [&](auto& builder) {
        builder.SetDimensions(currentWindowWidth_, currentWindowHeight_);
    });

    if (!gtaoBlurFramebuffer_) {
        throw std::runtime_error("Failed to create framebuffer (for GTAO blur)!");
    }
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // GTAO Pass
    {
        currentCmdBuffer->BeginRenderPass(
                [&](auto& beginInfo) {
                    beginInfo.renderPass = gtaoRenderPass_->GetHandle();
                    beginInfo.framebuffer = gtaoFramebuffer_->GetHandle();
                    beginInfo.renderArea.offset = {0, 0};
                    beginInfo.renderArea.extent = VkExtent2D(currentWindowWidth_, currentWindowHeight_);
        ...
        const std::vector gtaoDescSets{resources_->GetDescriptorSet(kGtaoDescSet)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, gtaoPipelineLayout_, 0, gtaoDescSets);
        currentCmdBuffer->BindPipeline(gtaoPassPipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);

        GtaoPushConstants pushConstants{};
        pushConstants.projection = camera_->GetProjectionMatrix();
        pushConstants.radius = gtaoRadius_;
        pushConstants.thickness = gtaoThickness_;
        pushConstants.power = gtaoPower_;
        pushConstants.sliceCount = kGtaoSliceCount;
        pushConstants.stepsPerSlice = kGtaoStepPerSlice;
        currentCmdBuffer->PushConstants(gtaoPipelineLayout_, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(pushConstants),
                                        &pushConstants);
    ...
    // GTAO Blur Pass
    {
        currentCmdBuffer->BeginRenderPass(
                [&](auto& beginInfo) {
                    beginInfo.renderPass = gtaoRenderPass_->GetHandle();
                    beginInfo.framebuffer = gtaoBlurFramebuffer_->GetHandle();
                    beginInfo.renderArea.offset = {0, 0};
                    beginInfo.renderArea.extent = VkExtent2D(currentWindowWidth_, currentWindowHeight_);
                    beginInfo.clearValueCount = occlusionPassClearValues.size();
                    beginInfo.pClearValues = occlusionPassClearValues.data();
        ...
        const std::vector blurDescSets{resources_->GetDescriptorSet(kGtaoBlurDescSet)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, gtaoBlurPipelineLayout_, 0, blurDescSets);
        currentCmdBuffer->BindPipeline(gtaoBlurPassPipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
    ...
    // Light Pass
    {
        ...
        lightingPushConstants.gtaoEnabled = isGtaoEnabled_ ? 1U : 0U;
