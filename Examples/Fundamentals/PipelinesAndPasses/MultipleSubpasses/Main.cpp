/**
 * @file    Main.cpp
 * @brief   In this example, three cubes rotating at different speeds are drawn on the screen, and the depth buffer
 *          data for these three cubes is drawn visually on the fourth cube that drawn on the right side of the screen.
 *          Input attachment and multiple subpasses are used for this purpose.
...
using namespace examples::fundamentals::pipelines_and_passes::multiple_subpasses;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::ObjectDescSetLayout, "objectDescSetLayout");
    schema.RegisterImmutableParam<std::string>(AppConstants::DepthObjectDescSetLayout, "depthObjectDescSetLayout");
