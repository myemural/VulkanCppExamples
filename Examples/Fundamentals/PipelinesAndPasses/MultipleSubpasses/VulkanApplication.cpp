...
void VulkanApplication::CreatePipeline()
{
    ...
    pipelineLayoutObject_ = device_->CreatePipelineLayout({
                                                              resources_->GetDescriptorLayout(
                                                                  GetParamStr(AppConstants::ObjectDescSetLayout))
                                                          },
                                                          {mvpPushConstant});
    ...
    pipelineLayoutDepthObject_ = device_->CreatePipelineLayout({
                                                                   resources_->GetDescriptorLayout(
                                                                       GetParamStr(
                                                                           AppConstants::DepthObjectDescSetLayout))
                                                               },
                                                               {mvpPushConstant});
    ...
    pipelineDepthObject_ = device_->CreateGraphicsPipeline(pipelineLayoutDepthObject_, renderPass_, [&](auto &builder) {
        ...
        builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(
                        GetParamStr(AppConstants::DepthObjectFragmentShaderKey))
                    ->GetHandle();
        });
        builder.SetVertexInputState([&](auto &vertexInputStateCreateInfo) {
            vertexInputStateCreateInfo.vertexBindingDescriptionCount = 1;
...
void VulkanApplication::UpdateDescriptorSets() const
{
    ...
    depthImageInfos.emplace_back(VK_NULL_HANDLE, resources_->GetImageView(GetParamStr(AppConstants::DepthImage),
                                                                          GetParamStr(AppConstants::DepthImageView))->
                                 GetHandle(),
                                 VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
                                                    ...
                                                    const std::uint32_t indexCount)
{
    ...
    cmdBuffersPresent_[currentImageIndex]->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayoutObject_, 0,
                                                              {
                                                                  resources_->GetDescriptorSet(
                                                                      GetParamStr(
                                                                          AppConstants::ObjectDescSetLayout))
                                                              });
