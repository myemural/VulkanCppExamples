...
void VulkanApplication::CreateResources()
{
    ...
    resourceCreateInfo.Buffers = {
        {
            GetParamStr(AppConstants::CubeVertexBuffer), cubeVertexBufferSize,
        ...
        {
            GetParamStr(AppConstants::PlaneVertexBuffer), planeVertexBufferSize,
            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
        },
        {
        ...
        {
            GetParamStr(AppConstants::CrateImageStagingBuffer), crateTextureHandler_.GetByteSize(),
            VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
        },
        {
            GetParamStr(AppConstants::CloudImageStagingBuffer), cloudTextureHandler_.GetByteSize(),
            VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT
        }
    };
    ...
    resourceCreateInfo.Shaders = {
        ...
        .Modules = {
            {
                .Name = GetParamStr(AppConstants::BackgroundVertexShaderKey),
    ...
    resourceCreateInfo.Images = {
        ...
        ImageResourceCreateInfo{
            .Name = GetParamStr(AppConstants::CloudImage),
            .MemProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            .Format = VK_FORMAT_R8G8B8A8_SRGB,
            .Dimensions = {cloudTextureHandler_.Width, cloudTextureHandler_.Height, 1},
            .Views = {
                ImageViewCreateInfo{
                    .ViewName = GetParamStr(AppConstants::CloudImageView),
                    .Format = VK_FORMAT_R8G8B8A8_SRGB
                }
            }
        },
...
void VulkanApplication::InitResources() const
{
    ...
    resources_->SetBuffer(GetParamStr(AppConstants::CrateImageStagingBuffer), crateTextureHandler_.Data,
                          crateTextureHandler_.GetByteSize());
    resources_->SetBuffer(GetParamStr(AppConstants::CloudImageStagingBuffer), cloudTextureHandler_.Data,
                          cloudTextureHandler_.GetByteSize());

    resources_->SetImageFromBuffer(cmdPool_, queue_, GetParamStr(AppConstants::CrateImage),
                                   resources_->GetBuffer(GetParamStr(AppConstants::CrateImageStagingBuffer)), {
                                       crateTextureHandler_.Width, crateTextureHandler_.Height, 1
                                   });
    resources_->SetImageFromBuffer(cmdPool_, queue_, GetParamStr(AppConstants::CloudImage),
                                   resources_->GetBuffer(GetParamStr(AppConstants::CloudImageStagingBuffer)), {
                                       cloudTextureHandler_.Width, cloudTextureHandler_.Height, 1
                                   });
...
void VulkanApplication::CreatePipelines()
{
    backgroundPipelineLayout_ = device_->CreatePipelineLayout({
        resources_->GetDescriptorLayout(
            GetParamStr(AppConstants::BackgroundDescSetLayout))
    });
    ...
    backgroundPipeline_ = device_->CreateGraphicsPipeline(backgroundPipelineLayout_, backgroundRenderPass_,
                                                          [&](auto &builder) {
                                                              builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
                                                                  shaderStageCreateInfo.stage =
                                                                          VK_SHADER_STAGE_VERTEX_BIT;
                                                                  shaderStageCreateInfo.module = resources_->
                                                                          GetShaderModule(
                                                                              GetParamStr(
                                                                                  AppConstants::BackgroundVertexShaderKey))
                                                                          ->
                                                                          GetHandle();
                                                              });
                                                              builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
                                                                  ...
                                                                  shaderStageCreateInfo.module = resources_->
                                                                          GetShaderModule(
                                                                              GetParamStr(
                                                                                  AppConstants::BackgroundFragmentShaderKey))
                                                                          ->GetHandle();
                                                              });
                                                              ...
                                                              builder.SetColorBlendState(
                                                                  [&](auto &blendStateCreateInfo) {
                                                                      blendStateCreateInfo.attachmentCount = 1;
                                                                      blendStateCreateInfo.pAttachments = &
    ...
    cubePipelineLayout_ = device_->CreatePipelineLayout({
                                                            resources_->GetDescriptorLayout(
                                                                GetParamStr(AppConstants::CubeDescSetLayout))
                                                        }, {mvpPushConstant});
    ...
    cubePipeline_ = device_->CreateGraphicsPipeline(cubePipelineLayout_, foregroundRenderPass_, [&](auto &builder) {
        builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
            ...
            shaderStageCreateInfo.module = resources_->GetShaderModule(
                        GetParamStr(AppConstants::CubeVertexShaderKey))->
                    GetHandle();
        });
        builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(
                        GetParamStr(AppConstants::CubeFragmentShaderKey))
                    ->GetHandle();
        });
...
void VulkanApplication::CreateFramebuffers(const std::shared_ptr<VulkanImageView> &depthImageView)
{
    for (const auto &swapImage: swapChainImageViews_) {
        const auto backgroundFramebuffer = device_->CreateFramebuffer(backgroundRenderPass_,
                                                                      {swapImage, depthImageView}, [&](auto &builder) {
                                                                          builder.SetDimensions(
                                                                              currentWindowWidth_,
                                                                              currentWindowHeight_);
                                                                      });
        ...
        const auto foregroundFramebuffer = device_->CreateFramebuffer(foregroundRenderPass_,
                                                                      {swapImage, depthImageView}, [&](auto &builder) {
                                                                          builder.SetDimensions(
                                                                              currentWindowWidth_,
                                                                              currentWindowHeight_);
...
void VulkanApplication::UpdateDescriptorSets() const
{
    ...
    backgroundSamplerInfos.emplace_back(
        ...
        resources_->GetImageView(GetParamStr(AppConstants::CloudImage),
                                 GetParamStr(AppConstants::CloudImageView))->GetHandle(),
    ...
    cubeSamplerInfos.emplace_back(
        resources_->GetSampler(GetParamStr(AppConstants::MainSampler))->GetHandle(),
        resources_->GetImageView(GetParamStr(AppConstants::CrateImage),
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // First render pass for background
    {
        ...
        cmdBuffersPresent_[currentImageIndex]->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS,
                                                                  backgroundPipelineLayout_, 0,
                                                                  {
                                                                      resources_->GetDescriptorSet(
                                                                          GetParamStr(
                                                                              AppConstants::BackgroundDescSetLayout))
                                                                  });
        cmdBuffersPresent_[currentImageIndex]->BindVertexBuffers({
                                                                     resources_->GetBuffer(GetParamStr(
                                                                         AppConstants::PlaneVertexBuffer))
                                                                 }, 0, 1, {0});
        cmdBuffersPresent_[currentImageIndex]->BindIndexBuffer(
            resources_->GetBuffer(GetParamStr(AppConstants::PlaneIndexBuffer)), 0, VK_INDEX_TYPE_UINT16);


        cmdBuffersPresent_[currentImageIndex]->BindPipeline(backgroundPipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
    ...
    // Second render pass for foreground
    {
        ...
        cmdBuffersPresent_[currentImageIndex]->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, cubePipelineLayout_,
                                                                  0,
                                                                  {
                                                                      resources_->GetDescriptorSet(
                                                                          GetParamStr(
                                                                              AppConstants::CubeDescSetLayout))
                                                                  });
        cmdBuffersPresent_[currentImageIndex]->BindVertexBuffers({
                                                                     resources_->GetBuffer(GetParamStr(
                                                                         AppConstants::CubeVertexBuffer))
                                                                 }, 0, 1, {0});
        cmdBuffersPresent_[currentImageIndex]->BindIndexBuffer(
            resources_->GetBuffer(GetParamStr(AppConstants::CubeIndexBuffer)), 0, VK_INDEX_TYPE_UINT16);


        cmdBuffersPresent_[currentImageIndex]->BindPipeline(cubePipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);

        for (size_t i = 0; i < NUM_CUBES; i++) {
            cmdBuffersPresent_[currentImageIndex]->PushConstants(cubePipelineLayout_, VK_SHADER_STAGE_VERTEX_BIT, 0,
                                                                 sizeof(MvpData), &cubeMvpData[i]);
