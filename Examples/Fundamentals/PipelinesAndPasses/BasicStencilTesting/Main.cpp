...
using namespace examples::fundamentals::pipelines_and_passes::basic_stencil_testing;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::OutlineFragmentShaderFile, "drawing_outline.frag.spv");
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::OutlineFragmentShaderKey, "fragOutline");
    ...
    schema.RegisterParam<float>(AppSettings::OutlineWidth);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::OutlineWidth, 0.05f);
