...
void VulkanApplication::CreateResources()
{
    ...
    const auto objectFragmentShaderAsset = assetManager_->Load<ShaderAsset>(kObjectFragmentShaderFile);
    const auto outlineFragmentShaderAsset = assetManager_->Load<ShaderAsset>(kOutlineFragmentShaderFile);

    resourceCreateInfo.shaders = {
        .modules = {{.name = kMainVertexShaderKey, .asset = assetManager_->Get(mainVertexShaderAsset)},
                    {.name = kObjectFragmentShaderKey, .asset = assetManager_->Get(objectFragmentShaderAsset)},
                    {.name = kOutlineFragmentShaderKey, .asset = assetManager_->Get(outlineFragmentShaderAsset)}}};
...
void VulkanApplication::CreatePipeline()
{
    ...
    VkStencilOpState stencilOpNormal = {.failOp = VK_STENCIL_OP_KEEP,
    ...
    VkStencilOpState stencilOpOutline = {.failOp = VK_STENCIL_OP_KEEP,
    ...
    pipelineObject_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto& builder) {
        ...
        builder.SetDepthStencilState([&](auto& depthStencilStateCreateInfo) {
            ...
            depthStencilStateCreateInfo.front = stencilOpNormal;
            depthStencilStateCreateInfo.back = stencilOpNormal;
        });
    });
    ...
    pipelineOutline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto& builder) {
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kMainVertexShaderKey)->GetHandle();
        });
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kOutlineFragmentShaderKey)->GetHandle();
        });
        builder.SetVertexInputState([&](auto& vertexInputStateCreateInfo) {
            vertexInputStateCreateInfo.vertexBindingDescriptionCount = 1;
            vertexInputStateCreateInfo.pVertexBindingDescriptions = &bindingDescription;
        ...
        builder.SetDepthStencilState([&](auto& depthStencilStateCreateInfo) {
            ...
            depthStencilStateCreateInfo.depthCompareOp = VK_COMPARE_OP_LESS;
            depthStencilStateCreateInfo.stencilTestEnable = VK_TRUE;
            depthStencilStateCreateInfo.front = stencilOpOutline;
            depthStencilStateCreateInfo.back = stencilOpOutline;
        });
    });

    if (!pipelineOutline_) {
        throw std::runtime_error("Failed to create graphics pipeline(outline)!");
    }
}
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Draw objects
    for (auto& mvpObject: mvpData_) {
        currentCmdBuffer->BindPipeline(pipelineObject_, VK_PIPELINE_BIND_POINT_GRAPHICS);
        currentCmdBuffer->PushConstants(pipelineLayout_, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(MvpData), &mvpObject);
        currentCmdBuffer->DrawIndexed(indices.size(), 1, 0, 0, 0);
    }

    // Draw outlines
    for (auto& mvpOutline: mvpOutlineData_) {
        currentCmdBuffer->BindPipeline(pipelineOutline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
        currentCmdBuffer->PushConstants(pipelineLayout_, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(MvpData), &mvpOutline);
        currentCmdBuffer->DrawIndexed(indices.size(), 1, 0, 0, 0);
    }
...
void VulkanApplication::CalculateAndSetMvp()
{
    for (size_t i = 0; i < NUM_CUBES; i++) {
        auto modelObject = glm::mat4(1.0f);
        modelObject = glm::translate(modelObject, modelPositions[i]);

        auto modelOutline = glm::mat4(1.0f);
        modelOutline = glm::translate(modelOutline, modelPositions[i]);
        modelOutline = glm::scale(modelOutline, glm::vec3(1.0f) * (1.0f + GetParamFloat(AppSettings::OutlineWidth)));

        const glm::mat4 view = camera_->GetViewMatrix();
        glm::mat4 proj = camera_->GetProjectionMatrix();

        // Calculate MVP matrix
        mvpData_[i].mvpMatrix = proj * view * modelObject;
        mvpOutlineData_[i].mvpMatrix = proj * view * modelOutline;
    }
}
