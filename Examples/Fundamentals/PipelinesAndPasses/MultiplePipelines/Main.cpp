/**
 * @file    Main.cpp
 * @brief   In this example, multiple cubes are drawn in the scene. The first 3 cubes in the scene are drawn with a
 *          pipeline that gives red output, the last 3 cubes are drawn with a pipeline that gives blue output, and the
 *          remaining cubes are drawn with a pipeline that gives wireframe output. Additionally, the pipeline that
 *          gives blue output created with pipeline derivative.
...
using namespace examples::fundamentals::pipelines_and_passes::multiple_pipelines;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::RedFragmentShaderFile, "drawing_cube_red.frag.spv");
    schema.RegisterImmutableParam<std::string>(AppConstants::BlueFragmentShaderFile, "drawing_cube_blue.frag.spv");
    schema.RegisterImmutableParam<std::string>(AppConstants::MainVertexShaderKey, "vertMain");
    schema.RegisterImmutableParam<std::string>(AppConstants::RedFragmentShaderKey, "fragRed");
    schema.RegisterImmutableParam<std::string>(AppConstants::BlueFragmentShaderKey, "fragBlue");
