...
void VulkanApplication::CreateResources()
{
    ...
    resourceCreateInfo.Shaders = {
        ...
        .Modules = {
            ...
            {
                .Name = GetParamStr(AppConstants::RedFragmentShaderKey),
                .FileName = GetParamStr(AppConstants::RedFragmentShaderFile)
            },
            {
                .Name = GetParamStr(AppConstants::BlueFragmentShaderKey),
                .FileName = GetParamStr(AppConstants::BlueFragmentShaderFile)
            }
        }
    };
...
void VulkanApplication::CreatePipelines()
{
    ...
    basePipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto &builder) {
        builder.SetCreateFlags(VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT);
        ...
        builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
            ...
            shaderStageCreateInfo.module = resources_->GetShaderModule(
                        GetParamStr(AppConstants::RedFragmentShaderKey))
                    ->GetHandle();
        });
    ...
    if (!basePipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (base)!");
    }

    derivativePipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto &builder) {
        builder.SetCreateFlags(VK_PIPELINE_CREATE_DERIVATIVE_BIT);
        builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(
                        GetParamStr(AppConstants::MainVertexShaderKey))->
        ...
        builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(
                        GetParamStr(AppConstants::BlueFragmentShaderKey))
                    ->GetHandle();
        });
        builder.SetVertexInputState([&](auto &vertexInputStateCreateInfo) {
            vertexInputStateCreateInfo.vertexBindingDescriptionCount = 1;
        ...
        builder.SetBasePipeline(basePipeline_, -1);
    });

    if (!derivativePipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (derived)!");
    }
    ...
    wireframePipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto &builder) {
        builder.AddShaderStage([&](auto &shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(
                        GetParamStr(AppConstants::MainVertexShaderKey))->
    ...
    if (!wireframePipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (wireframe)!");
    }
}
                                                    ...
                                                    const std::uint32_t indexCount)
{
    ...
    for (size_t i = 0; i < NUM_CUBES; ++i) {
        if (i < 3) {
            cmdBuffersPresent_[currentImageIndex]->BindPipeline(basePipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
        } else if (i > 6) {
            cmdBuffersPresent_[currentImageIndex]->BindPipeline(derivativePipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
        } else {
            currentLineWidth_ = 2.0f + sin(static_cast<float>(glfwGetTime()));
            cmdBuffersPresent_[currentImageIndex]->SetLineWidth(currentLineWidth_);
            cmdBuffersPresent_[currentImageIndex]->BindPipeline(wireframePipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
        }
