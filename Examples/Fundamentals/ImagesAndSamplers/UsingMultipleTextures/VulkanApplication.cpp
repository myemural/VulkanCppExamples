...
void VulkanApplication::CreateResources()
{
    ...
    wallTextureHandler_ = textureLoader.Load(GetParamStr(AppConstants::WallTexturePath));
...
void VulkanApplication::InitResources()
{
    ...
    ChangeImageLayout(bricksTexImage_, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    ChangeImageLayout(wallTexImage_, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    CopyStagingBuffers();
    ChangeImageLayout(bricksTexImage_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ChangeImageLayout(wallTexImage_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
}
...
void VulkanApplication::CreateTextureImages()
{
    ...
    const uint32_t memoryTypeIndexBricks =
            physicalDevice_->FindMemoryType(memoryReqBricks.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    const uint32_t memoryTypeIndexWall =
            physicalDevice_->FindMemoryType(memoryReqWall.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
{
    bricksTexImageView_ = device_->CreateImageView(bricksTexImage_,
    ...
    wallTexImageView_ =
            device_->CreateImageView(wallTexImage_, [](auto& builder) { builder.SetFormat(VK_FORMAT_R8G8B8A8_SRGB); });
...
void VulkanApplication::CopyStagingBuffers()
{
    ...
    VkBufferImageCopy copyRegionBricks = {
        .bufferOffset = 0,
        .bufferRowLength = 0,
        .bufferImageHeight = 0,
        .imageSubresource =
    ...
    cmdBufferTransfer->CopyBufferToImage(buffers_[GetParamStr(AppConstants::WallStagingBuffer)]->GetBuffer(),
                                         wallTexImage_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, {copyRegionWall});
