...
{
    // Pre-load textures
    const auto leafTextureAssetHandler = assetManager_->Load<TextureAsset>(kLeafTexturePath);
    leafTextureAsset_ = assetManager_->Get(leafTextureAssetHandler);
    ...
    const std::vector<BufferResourceCreateInfo> bufferCreateInfos = {
        ...
        {kMainIndexBuffer, indexDataSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT},
        {kImageStagingBuffer, static_cast<std::uint32_t>(leafTextureAsset_.data.size()),
...
void VulkanApplication::InitResources()
{
    ...
    SetBuffer(kImageStagingBuffer, leafTextureAsset_.data.data(), leafTextureAsset_.data.size());

    UpdateDescriptorSets();

    ChangeImageLayout(leafTexImage_, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    CopyStagingBuffer();
    ChangeImageLayout(leafTexImage_, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
}
...
void VulkanApplication::CreateLeafTextureImage()
{
    leafTexImage_ = device_->CreateImage([&](auto& builder) {
        ...
        builder.SetDimensions(leafTextureAsset_.width, leafTextureAsset_.height);
    });
    ...
    const auto memoryReq = leafTexImage_->GetImageMemoryRequirements();

    const uint32_t memoryTypeIndex =
            physicalDevice_->FindMemoryType(memoryReq.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

    leafDeviceMemory_ = device_->AllocateMemory(memoryReq.size, memoryTypeIndex);

    if (!leafDeviceMemory_) {
        throw std::runtime_error("Failed to allocate texture device memory!");
    }

    leafTexImage_->BindImageMemory(leafDeviceMemory_, 0);
}

void VulkanApplication::CreateLeafTextureImageView()
{
    leafTexImageView_ =
            device_->CreateImageView(leafTexImage_, [](auto& builder) { builder.SetFormat(VK_FORMAT_R8G8B8A8_SRGB); });

    if (!leafTexImageView_) {
...
void VulkanApplication::CopyStagingBuffer()
{
    ...
    VkBufferImageCopy copyRegion = {
        ...
        .imageOffset = {0, 0, 0},
        .imageExtent = {leafTextureAsset_.width, leafTextureAsset_.height, 1},
    };
