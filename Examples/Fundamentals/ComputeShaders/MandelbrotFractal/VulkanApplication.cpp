...
void VulkanApplication::CreateResources()
{
    ...
    const auto mandelbrotComputeShaderAsset = assetManager_->Load<ShaderAsset>(kMandelbrotComputeShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {{.name = kMainVertexShaderKey, .asset = assetManager_->Get(mainVertexShaderAsset)},
                    ...
                    {.name = kMandelbrotComputeShaderKey, .asset = assetManager_->Get(mandelbrotComputeShaderAsset)}}};
    ...
    resourceCreateInfo.images = {ImageResourceCreateInfo{
        ...
        .views = {ImageViewCreateInfo{.viewName = kMandelbrotStorageImageView, .format = VK_FORMAT_R8G8B8A8_UNORM}}}};
...
void VulkanApplication::CreatePipelines()
{
    ...
    mandelbrotComputePipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kComputeDescSetLayout)}, {pushConstantRange});

    if (!mandelbrotComputePipelineLayout_) {
        throw std::runtime_error("Failed to create compute pipeline layout!");
    }

    mandelbrotComputePipeline_ = device_->CreateComputePipeline(mandelbrotComputePipelineLayout_, [&](auto& builder) {
        builder.SetShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kMandelbrotComputeShaderKey)->GetHandle();
        });
    });

    if (!mandelbrotComputePipeline_) {
...
void VulkanApplication::UpdateDescriptorSets() const
{
    ...
    imageSamplerInfos.emplace_back(
            ...
            resources_->GetImageView(kMandelbrotStorageImage, kMandelbrotStorageImageView)->GetHandle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    std::vector<VkDescriptorImageInfo> imageStorageInfos;
    imageStorageInfos.emplace_back(
            VK_NULL_HANDLE, resources_->GetImageView(kMandelbrotStorageImage, kMandelbrotStorageImageView)->GetHandle(),
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Compute phase
    {
        ...
        const auto mandelbrotStorageImage = resources_->GetImage(kMandelbrotStorageImage);
        const auto barrierUndefinedToGeneral = mandelbrotStorageImage->CreateImageMemoryBarrier(
        ...
        currentCmdBuffer->BindPipeline(mandelbrotComputePipeline_, VK_PIPELINE_BIND_POINT_COMPUTE);
        const std::vector descSets{resources_->GetDescriptorSet(kComputeDescSetLayout)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_COMPUTE, mandelbrotComputePipelineLayout_, 0,
                                             descSets);

        MandelbrotPushConstants pushConstants{};
        pushConstants.center = {-0.743643887037151f, 0.131825904205330f}; // "Seahorse Valley" area
        pushConstants.loopTime = GetParamFloat(AppSettings::LoopTime);
        pushConstants.zoomSpeed = GetParamFloat(AppSettings::ZoomSpeed);
        pushConstants.time = static_cast<float>(GetCurrentTime());
        currentCmdBuffer->PushConstants(mandelbrotComputePipelineLayout_, VK_SHADER_STAGE_COMPUTE_BIT, 0,
                                        sizeof(MandelbrotPushConstants), &pushConstants);
    ...
    // Render phase
    {
        ...
        const auto mandelbrotStorageImage = resources_->GetImage(kMandelbrotStorageImage);
        const auto barrierGeneralToShaderRead = mandelbrotStorageImage->CreateImageMemoryBarrier(
