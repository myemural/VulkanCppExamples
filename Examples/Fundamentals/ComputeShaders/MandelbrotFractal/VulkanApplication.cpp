...
void VulkanApplication::CreateResources()
{
    ...
    resourceCreateInfo.Shaders = {.BasePath = SHADERS_DIR,
                                  ...
                                  .Modules = {{.Name = GetParamStr(AppConstants::MainVertexShaderKey),
                                              ...
                                              {.Name = GetParamStr(AppConstants::MandelbrotComputeShaderKey),
                                               .FileName = GetParamStr(AppConstants::MandelbrotComputeShaderFile)}}};
    ...
    resourceCreateInfo.Images = {ImageResourceCreateInfo{
        .Name = GetParamStr(AppConstants::MandelbrotStorageImage),
        ...
        .Views = {ImageViewCreateInfo{.ViewName = GetParamStr(AppConstants::MandelbrotStorageImageView),
                                      .Format = VK_FORMAT_R8G8B8A8_UNORM}}}};
...
void VulkanApplication::CreatePipelines()
{
    ...
    mandelbrotComputePipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(GetParamStr(AppConstants::ComputeDescSetLayout))}, {pushConstantRange});
    ...
    mandelbrotComputePipeline_ = device_->CreateComputePipeline(mandelbrotComputePipelineLayout_, [&](auto& builder) {
        builder.SetShaderStage([&](auto& shaderStageCreateInfo) {
            ...
            shaderStageCreateInfo.module =
                    resources_->GetShaderModule(GetParamStr(AppConstants::MandelbrotComputeShaderKey))->GetHandle();
        });
    });
...
void VulkanApplication::UpdateDescriptorSets() const
{
    ...
    imageSamplerInfos.emplace_back(resources_->GetSampler(GetParamStr(AppConstants::MainSampler))->GetHandle(),
                                   resources_
                                           ->GetImageView(GetParamStr(AppConstants::MandelbrotStorageImage),
                                                          GetParamStr(AppConstants::MandelbrotStorageImageView))
                                           ->GetHandle(),
    ...
    storageUpdateRequest.LayoutName = GetParamStr(AppConstants::ComputeDescSetLayout);
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Compute phase
    {
        ...
        const auto mandelbrotStorageImage = resources_->GetImage(GetParamStr(AppConstants::MandelbrotStorageImage));
