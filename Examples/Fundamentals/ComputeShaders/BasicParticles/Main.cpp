/**
 * @file    Main.cpp
 * @brief   In this example, particles generated with a compute shader on the GPU are thrown from the cubes that drawn
 *          on the scene.
...
using namespace examples::fundamentals::compute_shaders::basic_particles;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::ParticleVertexShaderFile, "drawing_particles.vert.spv");
    schema.RegisterImmutableParam<std::string>(AppConstants::ParticleFragmentShaderFile, "drawing_particles.frag.spv");
    schema.RegisterImmutableParam<std::string>(AppConstants::ParticleComputeShaderFile,
                                               "particle_generator.comp.spv");
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::ParticleFragmentShaderKey, "fragParticle");
    schema.RegisterImmutableParam<std::string>(AppConstants::ParticleComputeShaderKey, "compParticle");
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::ParticleStorageBuffer, "particleStorageBuffer");
    ...
    schema.RegisterParam<std::uint32_t>(AppSettings::ParticleCount);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set<std::uint32_t>(AppSettings::ParticleCount, 120);
