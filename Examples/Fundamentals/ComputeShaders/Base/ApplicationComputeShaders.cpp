...
void ApplicationComputeShaders::SetWindow(const std::shared_ptr<Window>& window) { window_ = window; }

void ApplicationComputeShaders::PreUpdate()
{
...
bool ApplicationComputeShaders::ShouldClose() { return window_->CheckWindowCloseFlag(); }

void ApplicationComputeShaders::CreateDefaultSurface()
{
...
void ApplicationComputeShaders::SelectDefaultPhysicalDevice()
{
...
void ApplicationComputeShaders::CreateDefaultLogicalDevice()
{
...
void ApplicationComputeShaders::CreateDefaultQueue() { queue_ = device_->CreateQueue(currentQueueFamilyIndex_, 0); }

void ApplicationComputeShaders::CreateDefaultSwapChain()
{
...
void ApplicationComputeShaders::CreateDefaultFramebuffers(const std::shared_ptr<VulkanImageView>& depthImageView)
{
    const auto windowWidth = window_->GetWindowWidth();
    const auto windowHeight = window_->GetWindowHeight();
    ...
    for (const auto& swapImage: swapChainImageViews_) {
        ...
        attachments.emplace_back(swapImage);
        ...
        auto framebuffer = device_->CreateFramebuffer(
                renderPass_, attachments, [&](auto& builder) { builder.SetDimensions(windowWidth, windowHeight); });
...
void ApplicationComputeShaders::CreateDefaultCommandPool()
{
...
void ApplicationComputeShaders::CreateDefaultSyncObjects(const std::uint32_t maxFramesInFlight)
{
...
void ApplicationComputeShaders::CreateVulkanResources(const ResourceDescriptor& resourceCreateInfo)
{
