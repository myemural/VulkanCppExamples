...
void VulkanApplication::CreateResources()
{
    ...
    const std::uint32_t cubeIndexBufferSize = cubeIndices.size() * sizeof(cubeIndices[0]);
    const std::uint32_t uboSize = uboAlignedSize_ * NUM_CUBES;
    ...
    resourceCreateInfo.Buffers = {
        ...
        {GetParamStr(AppConstants::CubeIndexBuffer), cubeIndexBufferSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT},
        {GetParamStr(AppConstants::DynamicUniformBuffer), uboSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT}};
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    for (uint32_t i = 0; i < NUM_CUBES; ++i) {
        uint32_t dynamicOffset = i * uboAlignedSize_;
...
void VulkanApplication::CalculateAndSetMvp()
{
    ...
    resources_->SetBufferAlignedWithoutUnmap(GetParamStr(AppConstants::DynamicUniformBuffer), objectUbo_.data(),
                                             sizeof(ObjectUbo), NUM_CUBES, uboAlignedSize_);
}
