/**
 * @file    Main.cpp
 * @brief   In this example, three different variants of a fragment shader are created using specialization constants,
 *          and three separate pipelines are created from these fragment shaders. Then drawing is done on the screen
...
using namespace examples::fundamentals::queries_and_performance::specialization_constants;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::MainFragmentShaderFile,
                                               "texture_sampler_specialized.frag.spv");
