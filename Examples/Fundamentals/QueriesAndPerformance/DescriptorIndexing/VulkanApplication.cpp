...
void VulkanApplication::CreateResources()
{
    ...
    const auto wallStoneTextureAssetHandler = assetManager_->Load<TextureAsset>(kWallStoneTexturePath);
    wallStoneTextureAsset_ = assetManager_->Get(wallStoneTextureAssetHandler);
    ...
    resourceCreateInfo.descriptors = {
        .maxSets = 1 + TOTAL_TEXTURE_COUNT,
        .poolSizes = {{VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 1},
                      {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, TOTAL_TEXTURE_COUNT}},
        .layouts = {{.name = kMainDescSetLayout,
                     .bindings = {{0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC, 1, VK_SHADER_STAGE_VERTEX_BIT,
                                   nullptr},
                                  {1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, TOTAL_TEXTURE_COUNT,
    ...
    resourceCreateInfo.images = {
        ...
        ImageResourceCreateInfo{
            .name = kWallStoneImage,
            .memProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            .format = VK_FORMAT_R8G8B8A8_SRGB,
            .dimensions = {wallStoneTextureAsset_.width, wallStoneTextureAsset_.height, 1},
            .views = {ImageViewCreateInfo{.viewName = kWallStoneImageView, .format = VK_FORMAT_R8G8B8A8_SRGB}}},
        ImageResourceCreateInfo{
            .name = kDepthImage,
            .memProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
...
void VulkanApplication::InitResources() const
{
    ...
    resources_->SetImageFromTexture(cmdPool_, queue_, kWallStoneImage, wallStoneTextureAsset_);
...
void VulkanApplication::UpdateDescriptorSets() const
{
    ...
    std::vector<VkDescriptorImageInfo> imagesSamplerInfos;
    imagesSamplerInfos.emplace_back(resources_->GetSampler(kMainSampler)->GetHandle(),
                                    resources_->GetImageView(kCrateImage, kCrateImageView)->GetHandle(),
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    imagesSamplerInfos.emplace_back(resources_->GetSampler(kMainSampler)->GetHandle(),
                                    resources_->GetImageView(kWallStoneImage, kWallStoneImageView)->GetHandle(),
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    samplerUpdateRequestCube.images = imagesSamplerInfos;
