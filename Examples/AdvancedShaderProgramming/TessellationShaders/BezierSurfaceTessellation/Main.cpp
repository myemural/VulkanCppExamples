/**
 * @file    Main.cpp
 * @brief   In this example, a Bézier surface is drawn with 16 control points using tessellation shaders. The orbit
 *          camera is used, and the control points can be moved with the mouse.
...
using namespace examples::advanced_shader_programming::tessellation_shaders::bezier_surface_tessellation;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterParam<float>(AppSettings::CameraMoveSensitivity);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::CameraMoveSensitivity, 10.0f);
        params.Set(AppSettings::CameraZoomSpeed, 0.4f);
        params.Set(AppSettings::TessellationLevel, 32.0f);
        params.Set(AppSettings::ControlPointSize, 20.0f);
