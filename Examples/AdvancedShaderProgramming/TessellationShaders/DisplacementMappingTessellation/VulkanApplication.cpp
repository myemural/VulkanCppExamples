...
void VulkanApplication::BuildScene()
{
    ...
    sphereMaterial.ambientStrength = 0.1f;
    sphereMaterial.specularStrength = 0.8f;
    sphereMaterial.shininess = 128.0f;
    sphereMaterial.diffuseMap = metalPatternTextureId;
    sphereMaterial.normalMap = metalPatternNormalTextureId;
    sphereMaterial.heightMap = metalPatternHeightTextureId;

    Material planeMaterial;
    planeMaterial.ambientStrength = 0.1f;
    planeMaterial.specularStrength = 0.35f;
    planeMaterial.shininess = 32.0f;
    planeMaterial.diffuseMap = wallStoneTextureId;
    planeMaterial.normalMap = wallStoneNormalTextureId;
    planeMaterial.heightMap = wallStoneHeightTextureId;
    ...
    const auto rootObject = SceneObjectBuilder(*scene_, kRootObject)
                                    ...
                                    .AddChild(SceneObjectBuilder(*scene_, kSphereObject2)
                                                      ...
                                                      .WithPosition(glm::vec3{2.0f, 0.0f, -2.0f})
                                                      .WithScale(glm::vec3{2.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kSphereObject3)
                                                      .WithTag(kSphericalObject)
                                                      .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                                      .WithMaterial(sphereMaterial)
                                                      .WithPosition(glm::vec3{-2.0f, 0.0f, 2.0f})
                                                      .WithScale(glm::vec3{2.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kSphereObject4)
                                                      .WithTag(kSphericalObject)
                                                      .WithBuiltinMesh(BuiltinMeshType::SPHERE)
                                                      .WithMaterial(sphereMaterial)
                                                      .WithPosition(glm::vec3{2.0f, 0.0f, 2.0f})
                                                      .WithScale(glm::vec3{2.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kPlane)
...
void VulkanApplication::CreatePipelines()
{
    ...
    scenePipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto& builder) {
        ...
        builder.SetTessellationState(
                [&](auto& tessellationStateCreateInfo) { tessellationStateCreateInfo.patchControlPoints = 3; });
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    scene_->Traverse([&](const SceneObject& sceneObject) {
        if (sceneObject.HasRenderable()) {
            ...
            meshPushConstants.isSphericalObject = sceneObject.GetTag() == kSphericalObject ? 1U : 0U;
            meshPushConstants.tessellationLevel = sceneObject.GetTag() == kSphericalObject ? 8U : 32U;
            meshPushConstants.displacementLevel = sceneObject.GetTag() == kSphericalObject ? 0.05f : 0.025f;
