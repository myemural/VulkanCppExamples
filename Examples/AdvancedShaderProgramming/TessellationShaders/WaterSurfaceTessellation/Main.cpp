...
using namespace examples::advanced_shader_programming::tessellation_shaders::water_surface_tessellation;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterParam<float>(AppSettings::TessLevel);
    schema.RegisterParam<float>(AppSettings::DisplacementLevel);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::TessLevel, 32.0f);
        params.Set(AppSettings::DisplacementLevel, 1.0f);
