...
void VulkanApplication::BuildScene()
{
    ...
    waterMaterial.ambientStrength = 0.05f;
    waterMaterial.specularStrength = 0.7f;
    waterMaterial.shininess = 256.0f;
    waterMaterial.normalMap = waterNormalTextureId;
    waterMaterial.uvScale = 32.0f;

    const auto rootObject = SceneObjectBuilder(*scene_, kRootObject)
                                    .WithPosition(glm::vec3{0.0f, 0.0f, 0.0f})
                                    .AddChild(SceneObjectBuilder(*scene_, kWaterSurface)
                                                      .WithBuiltinMesh(BuiltinMeshType::TESSELLATED_PLANE)
                                                      .WithMaterial(waterMaterial)
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    scene_->Traverse([&](const SceneObject& sceneObject) {
        if (sceneObject.HasRenderable()) {
            ...
            pushConstants.objectId = sceneObject.GetObjectId();
            ...
            pushConstants.tessellationLevel = GetParamFloat(AppSettings::TessLevel);
            pushConstants.displacementLevel = GetParamFloat(AppSettings::DisplacementLevel);
