...
void VulkanApplication::CreateInitialResources() const
{
    ...
    resourceCreateInfo.samplers = {
        ...
        SamplerResourceCreateInfo{.name = kTerrainSampler,
                                  .filtering = {.magFilter = VK_FILTER_LINEAR, .minFilter = VK_FILTER_LINEAR},
...
void VulkanApplication::BuildScene()
{
    ...
    sceneConfig.primitiveStackCount = 8U;
    sceneConfig.primitiveSectorCount = 8U;
    ...
    const auto heightmapTextureAsset = assetManager_->Load<TextureAsset>(kHeightmapTexturePath);
    const auto heightmapTextureId = sceneImageStorage.StoreTexture(
            kHeightmapTexture, kTerrainSampler, assetManager_->Get(heightmapTextureAsset), VK_FORMAT_R8G8B8A8_UNORM);

    Material planeMaterial;
    planeMaterial.ambientStrength = 0.1f;
    planeMaterial.specularStrength = 0.08f;
    planeMaterial.shininess = 12.0f;
    planeMaterial.diffuseMap = groundTextureId;
    planeMaterial.normalMap = groundNormalTextureId;
    planeMaterial.heightMap = heightmapTextureId;
    planeMaterial.uvScale = 8.0f;

    const auto rootObject = SceneObjectBuilder(*scene_, kRootObject)
                                    .WithPosition(glm::vec3{0.0f, 0.0f, 0.0f})
                                    .AddChild(SceneObjectBuilder(*scene_, kTerrain)
                                                      .WithBuiltinMesh(BuiltinMeshType::TESSELLATED_PLANE)
                                                      .WithMaterial(planeMaterial)
                                                      .WithPosition(glm::vec3{0.0f, -3.0f, 0.0f})
                                                      .WithScale(glm::vec3{32.0f}))
                                    .Build();
...
void VulkanApplication::CreatePipelines()
{
    ...
    scenePipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto& builder) {
        ...
        builder.SetTessellationState(
                [&](auto& tessellationStateCreateInfo) { tessellationStateCreateInfo.patchControlPoints = 4; });
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    scene_->Traverse([&](const SceneObject& sceneObject) {
        if (sceneObject.HasRenderable()) {
            ...
            meshPushConstants.enablePatchLines = enablePatchLines_ ? 1U : 0U;
            meshPushConstants.displacementLevel = 0.15f;
            meshPushConstants.minTessDistance = GetParamFloat(AppSettings::MinTessDistance);
            meshPushConstants.maxTessDistance = GetParamFloat(AppSettings::MaxTessDistance);
            meshPushConstants.minTessLevel = GetParamFloat(AppSettings::MinTessLevel);
            meshPushConstants.maxTessLevel = GetParamFloat(AppSettings::MaxTessLevel);
...
void VulkanApplication::ProcessInput()
{
    ...
    // Enable/disable patch line visualization
    if (window_->IsKeyPressed(GLFW_KEY_0)) {
        enablePatchLines_ = false;
    }
    if (window_->IsKeyPressed(GLFW_KEY_1)) {
        enablePatchLines_ = true;
    }
}
} // namespace examples::advanced_shader_programming::tessellation_shaders::tessellated_terrain_dynamic_lod
