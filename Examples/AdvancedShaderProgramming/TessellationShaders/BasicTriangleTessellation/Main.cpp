/**
 * @file    Main.cpp
 * @brief   In this example, a simple triangle is drawn on the screen in wireframe mode, and tessellation shaders are
 *          applied to this triangle, dividing its interior into smaller triangles. The inner tessellation level value
 *          is set by the user.
...
using namespace examples::advanced_shader_programming::tessellation_shaders::basic_triangle_tessellation;
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::LineWidth, 2.0f);
