...
void ApplicationTessellationShaders::SetWindow(const std::shared_ptr<Window>& window) { window_ = window; }

bool ApplicationTessellationShaders::Init()
{
    try {
...
void ApplicationTessellationShaders::PreUpdate()
{
...
void ApplicationTessellationShaders::PostUpdate() { window_->SwapBuffers(); }

bool ApplicationTessellationShaders::ShouldClose() { return window_->CheckWindowCloseFlag(); }

void ApplicationTessellationShaders::CreateDefaultSurface()
{
...
void ApplicationTessellationShaders::SelectDefaultPhysicalDevice()
{
...
void ApplicationTessellationShaders::CreateDefaultLogicalDevice()
{
...
void ApplicationTessellationShaders::CreateDefaultQueue()
{
    queue_ = device_->CreateQueue(currentQueueFamilyIndex_, 0);
}

void ApplicationTessellationShaders::CreateDefaultSwapChain()
{
...
void ApplicationTessellationShaders::CreateDefaultFramebuffers(const std::shared_ptr<VulkanImageView>& depthImageView)
{
    const auto windowWidth = window_->GetWindowWidth();
    const auto windowHeight = window_->GetWindowHeight();
...
void ApplicationTessellationShaders::CreateDefaultCommandPool()
{
...
void ApplicationTessellationShaders::CreateDefaultSyncObjects()
{
...
void ApplicationTessellationShaders::CreateVulkanResources(const ResourceDescriptor& resourceCreateInfo) const
{
