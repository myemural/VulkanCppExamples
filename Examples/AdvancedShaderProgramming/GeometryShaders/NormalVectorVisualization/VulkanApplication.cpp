...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto normalVisualVertexShaderAsset = assetManager_->Load<ShaderAsset>(kNormalVisualVertexShaderFile);
    const auto normalVisualGeometryShaderAsset = assetManager_->Load<ShaderAsset>(kNormalVisualGeometryShaderFile);
    const auto normalVisualFragmentShaderAsset = assetManager_->Load<ShaderAsset>(kNormalVisualFragmentShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {
            ...
            {.name = kNormalVisualVertexShaderKey, .asset = assetManager_->Get(normalVisualVertexShaderAsset)},
            {.name = kNormalVisualGeometryShaderKey, .asset = assetManager_->Get(normalVisualGeometryShaderAsset)},
            {.name = kNormalVisualFragmentShaderKey, .asset = assetManager_->Get(normalVisualFragmentShaderAsset)}}};
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        ...
        .layouts = {{.name = kMainDescSetLayout,
                    ...
                    {.name = kNormalVisualDescSetLayout,
                     .bindings = {{0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT, nullptr}}}},
        .descriptorSets = {{.name = kMainDescSet, .layoutName = kMainDescSetLayout},
                           {.name = kNormalVisualDescSet, .layoutName = kNormalVisualDescSetLayout}}};
    ...
    BufferWriteRequest transformBufferRequestForNormalVisual;
    transformBufferRequestForNormalVisual.descriptorSetName = kNormalVisualDescSet;
    transformBufferRequestForNormalVisual.bindingIndex = 0;
    transformBufferRequestForNormalVisual.buffers = storageTransformBufferInfos;
    transformBufferRequestForNormalVisual.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;

    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        .bufferWriteRequests = {objectStorageTransformBufferRequest, objectStorageMaterialBufferRequest,
                                lightUboRequest, transformBufferRequestForNormalVisual},
...
void VulkanApplication::CreatePipelines()
{
    ...
    VkPushConstantRange normalVisualPushConstant;
    normalVisualPushConstant.offset = 0;
    normalVisualPushConstant.size = sizeof(NormalVisualPushConstants);
    normalVisualPushConstant.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_GEOMETRY_BIT;

    normalVisualPipelineLayout_ = device_->CreatePipelineLayout({resources_->GetDescriptorLayout(kNormalVisualDescSetLayout)},
                                                                {normalVisualPushConstant});

    if (!normalVisualPipelineLayout_) {
        throw std::runtime_error("Failed to create pipeline layout (for normal vector visualization)!");
    }

    normalVisualPipeline_ =
            device_->CreateGraphicsPipeline(normalVisualPipelineLayout_, renderPass_, [&](auto& builder) {
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
                    shaderStageCreateInfo.module =
                            resources_->GetShaderModule(kNormalVisualVertexShaderKey)->GetHandle();
                });
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_GEOMETRY_BIT;
                    shaderStageCreateInfo.module =
                            resources_->GetShaderModule(kNormalVisualGeometryShaderKey)->GetHandle();
                });
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
                    shaderStageCreateInfo.module =
                            resources_->GetShaderModule(kNormalVisualFragmentShaderKey)->GetHandle();
                });
                builder.SetVertexInputState([&](auto& vertexInputStateCreateInfo) {
                    vertexInputStateCreateInfo.vertexBindingDescriptionCount = bindings.size();
                    vertexInputStateCreateInfo.pVertexBindingDescriptions = bindings.data();
    ...
    if (!normalVisualPipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (for normal vector visualization)!");
    }
}
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    if (enableNormalDebug_) {
        const std::vector normalVisualDescSets{resources_->GetDescriptorSet(kNormalVisualDescSet)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, normalVisualPipelineLayout_, 0,
                                             normalVisualDescSets);
        currentCmdBuffer->BindPipeline(normalVisualPipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
        scene_->Traverse([&](const SceneObject& sceneObject) {
            if (sceneObject.HasRenderable()) {
                const auto [vertexOffsets, indexOffset, indexCount] = sceneObject.GetMeshGpu().value();
                currentCmdBuffer->BindVertexBuffers(vertexBuffers, 0, vertexBuffers.size(), vertexOffsets);
                ...
                NormalVisualPushConstants normalVisualPushConstants{};
                normalVisualPushConstants.objectId = sceneObject.GetObjectId();
                normalVisualPushConstants.view = camera_->GetViewMatrix();
                normalVisualPushConstants.projection = camera_->GetProjectionMatrix();
                currentCmdBuffer->PushConstants(normalVisualPipelineLayout_,
                                                VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_GEOMETRY_BIT, 0,
                                                sizeof(normalVisualPushConstants), &normalVisualPushConstants);
                currentCmdBuffer->DrawIndexed(indexCount, 1, 0, 0, 0);
            }
        });
    }
...
void VulkanApplication::ProcessInput()
{
    ...
    if (window_->IsKeyPressed(GLFW_KEY_0)) {
        enableNormalDebug_ = false;
    }
    if (window_->IsKeyPressed(GLFW_KEY_1)) {
        enableNormalDebug_ = true;
    }
}
} // namespace examples::advanced_shader_programming::geometry_shaders::normal_vector_visualization
