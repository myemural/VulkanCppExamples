/**
 * @file    Main.cpp
 * @brief   In this example, wireframe overlay visualization is performed on objects in the scene using the geometry
 *          shader and depth bias.
...
using namespace examples::advanced_shader_programming::geometry_shaders::wireframe_overlay_visualization;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterParam<float>(AppSettings::WireframeLineWidth);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::WireframeLineWidth, 1.5f);
