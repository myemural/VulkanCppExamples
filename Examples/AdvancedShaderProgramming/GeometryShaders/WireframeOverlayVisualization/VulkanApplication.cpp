...
VulkanApplication::VulkanApplication(ParameterServer&& params) : ApplicationGeometryShaders(std::move(params)) {}

bool VulkanApplication::Init()
{
    if (!ApplicationGeometryShaders::Init()) {
...
{
    // Poll events
    ApplicationGeometryShaders::PreUpdate();
...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto wireframeVisualVertexShaderAsset = assetManager_->Load<ShaderAsset>(kWireframeVisualVertexShaderFile);
    const auto wireframeVisualGeometryShaderAsset =
            assetManager_->Load<ShaderAsset>(kWireframeVisualGeometryShaderFile);
    const auto wireframeVisualFragmentShaderAsset =
            assetManager_->Load<ShaderAsset>(kWireframeVisualFragmentShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {
            ...
            {.name = kWireframeVisualVertexShaderKey, .asset = assetManager_->Get(wireframeVisualVertexShaderAsset)},
            {.name = kWireframeVisualGeometryShaderKey,
             .asset = assetManager_->Get(wireframeVisualGeometryShaderAsset)},
            {.name = kWireframeVisualFragmentShaderKey,
             .asset = assetManager_->Get(wireframeVisualFragmentShaderAsset)}}};
    ...
    resourceCreateInfo.samplers = {SamplerResourceCreateInfo{
...
void VulkanApplication::BuildScene()
{
    ...
    const auto milkTruckModelHandle = assetManager_->Load<GltfModelAsset>(kMilkTruckModelPath);
    const auto milkTruckModel = std::make_unique<GltfModelAsset>(assetManager_->Get(milkTruckModelHandle));

    GltfToSceneObjectConverter converter{*scene_, kMainSampler};
    auto milkTruckModelSceneObjectBuilder = converter.ConvertToBuilder(kMilkTruckModelName, *milkTruckModel);
    ...
    for (auto i = 0; i < 8; ++i) {
        ...
        const auto zShift = -static_cast<float>(i * 2 - 1) + 6.0f;
        rootObjectBuilder
    ...
    const auto& rootObject =
            rootObjectBuilder
                    .AddChild(milkTruckModelSceneObjectBuilder.WithPosition(glm::vec3{0.0f, -2.0f, 0.0f})
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        ...
        .layouts = {{.name = kMainDescSetLayout,
                    ...
                    {.name = kWireframeVisualDescSetLayout,
                     .bindings = {{0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT, nullptr}}}},
        .descriptorSets = {{.name = kMainDescSet, .layoutName = kMainDescSetLayout},
                           {.name = kWireframeVisualDescSet, .layoutName = kWireframeVisualDescSetLayout}}};
    ...
    BufferWriteRequest transformBufferRequestForWireframeVisual;
    transformBufferRequestForWireframeVisual.descriptorSetName = kWireframeVisualDescSet;
    transformBufferRequestForWireframeVisual.bindingIndex = 0;
    transformBufferRequestForWireframeVisual.buffers = storageTransformBufferInfos;
    transformBufferRequestForWireframeVisual.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;

    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        .bufferWriteRequests = {objectStorageTransformBufferRequest, objectStorageMaterialBufferRequest,
                                lightUboRequest, transformBufferRequestForWireframeVisual},
...
void VulkanApplication::CreatePipelines()
{
    ...
    VkPushConstantRange wireframeVisualPushConstant;
    wireframeVisualPushConstant.offset = 0;
    wireframeVisualPushConstant.size = sizeof(WireframeVisualPushConstants);
    wireframeVisualPushConstant.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_GEOMETRY_BIT;

    wireframeVisualPipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kWireframeVisualDescSetLayout)}, {wireframeVisualPushConstant});

    if (!wireframeVisualPipelineLayout_) {
        throw std::runtime_error("Failed to create pipeline layout (for wireframe overlay visualization)!");
    }

    wireframeVisualPipeline_ =
            device_->CreateGraphicsPipeline(wireframeVisualPipelineLayout_, renderPass_, [&](auto& builder) {
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
                    shaderStageCreateInfo.module =
                            resources_->GetShaderModule(kWireframeVisualVertexShaderKey)->GetHandle();
                });
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_GEOMETRY_BIT;
                    shaderStageCreateInfo.module =
                            resources_->GetShaderModule(kWireframeVisualGeometryShaderKey)->GetHandle();
                });
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
                    shaderStageCreateInfo.module =
                            resources_->GetShaderModule(kWireframeVisualFragmentShaderKey)->GetHandle();
                });
                builder.SetVertexInputState([&](auto& vertexInputStateCreateInfo) {
                    vertexInputStateCreateInfo.vertexBindingDescriptionCount = bindings.size();
                    vertexInputStateCreateInfo.pVertexBindingDescriptions = bindings.data();
                ...
                builder.SetRasterizationState([&](auto& rasterizationStateCreateInfo) {
                    ...
                    rasterizationStateCreateInfo.depthBiasConstantFactor = -1.0f;
                    rasterizationStateCreateInfo.depthBiasSlopeFactor = -1.0f; // Extra bias on sloped surfaces
                    rasterizationStateCreateInfo.depthBiasClamp = 0.0f;
                    rasterizationStateCreateInfo.lineWidth = GetParamFloat(AppSettings::WireframeLineWidth);
                });
                builder.SetColorBlendState([&](auto& blendStateCreateInfo) {
                    blendStateCreateInfo.attachmentCount = 1;
                    blendStateCreateInfo.pAttachments = &colorBlendAttachment;
    ...
    if (!wireframeVisualPipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (for wireframe overlay visualization)!");
    }
}
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    if (enableWireframeDebug_) {
        const std::vector wireframeVisualDescSets{resources_->GetDescriptorSet(kWireframeVisualDescSet)};
        currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, wireframeVisualPipelineLayout_, 0,
                                             wireframeVisualDescSets);
        currentCmdBuffer->BindPipeline(wireframeVisualPipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
        scene_->Traverse([&](const SceneObject& sceneObject) {
            if (sceneObject.HasRenderable()) {
                const auto [vertexOffsets, indexOffset, indexCount] = sceneObject.GetMeshGpu().value();
                currentCmdBuffer->BindVertexBuffers(vertexBuffers, 0, vertexBuffers.size(), vertexOffsets);
                ...
                WireframeVisualPushConstants wireframeVisualPushConstants{};
                wireframeVisualPushConstants.objectId = sceneObject.GetObjectId();
                wireframeVisualPushConstants.view = camera_->GetViewMatrix();
                wireframeVisualPushConstants.projection = camera_->GetProjectionMatrix();
                currentCmdBuffer->PushConstants(wireframeVisualPipelineLayout_,
                                                VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_GEOMETRY_BIT, 0,
                                                sizeof(wireframeVisualPushConstants), &wireframeVisualPushConstants);
                currentCmdBuffer->DrawIndexed(indexCount, 1, 0, 0, 0);
            }
        });
    }
...
void VulkanApplication::ProcessInput()
{
    ...
    // Enable/disable wireframe overlay visualization
    if (window_->IsKeyPressed(GLFW_KEY_0)) {
        enableWireframeDebug_ = false;
    }
    if (window_->IsKeyPressed(GLFW_KEY_1)) {
        enableWireframeDebug_ = true;
    }
}
} // namespace examples::advanced_shader_programming::geometry_shaders::wireframe_overlay_visualization
