...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto billboardingVertexAsset = assetManager_->Load<ShaderAsset>(kBillboardingVertexShaderFile);
    const auto billboardingGeometryAsset = assetManager_->Load<ShaderAsset>(kBillboardingGeometryShaderFile);
    const auto billboardingFragmentAsset = assetManager_->Load<ShaderAsset>(kBillboardingFragmentShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {{.name = kSceneVertexShaderKey, .asset = assetManager_->Get(sceneVertexShaderAsset)},
                    ...
                    {.name = kBillboardingVertexShaderKey, .asset = assetManager_->Get(billboardingVertexAsset)},
                    {.name = kBillboardingGeometryShaderKey, .asset = assetManager_->Get(billboardingGeometryAsset)},
                    {.name = kBillboardingFragmentShaderKey, .asset = assetManager_->Get(billboardingFragmentAsset)}}};
...
void VulkanApplication::BuildScene()
{
    ...
    const auto treeTextureAsset = assetManager_->Load<TextureAsset>(kTreeTexturePath);
    const auto treeTextureId =
            sceneImageStorage.StoreTexture(kTreeTexture, kMainSampler, assetManager_->Get(treeTextureAsset));

    Material groundMaterial;
    groundMaterial.ambientStrength = kAmbientStrength;
    groundMaterial.specularStrength = kSpecularStrength;
    groundMaterial.shininess = kSpecularShininess;
    groundMaterial.diffuseMap = groundTextureId;
    groundMaterial.normalMap = groundNormalTextureId;
    groundMaterial.uvScale = 15.0f;

    Material treeMaterial;
    treeMaterial.diffuseMap = treeTextureId;
    ...
    auto rootObjectBuilder = SceneObjectBuilder(*scene_, kRootObject)
                                     ...
                                     .AddChild(SceneObjectBuilder(*scene_, kGroundPlane)
                                                       ...
                                                       .WithPosition(glm::vec3{0.0f, -2.0f, -24.0f})
                                                       .WithScale(glm::vec3{60.0f}));

    int index = 0;
    for (const auto& pos: kBillboardPositions) {
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, kBillboardPoint + std::to_string(index++))
                                           .WithTag(kBillboardGroup)
...
void VulkanApplication::CreatePipelines()
{
    ...
    billboardingPipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto& builder) {
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kBillboardingVertexShaderKey)->GetHandle();
        });
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_GEOMETRY_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kBillboardingGeometryShaderKey)->GetHandle();
        });
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
            shaderStageCreateInfo.module = resources_->GetShaderModule(kBillboardingFragmentShaderKey)->GetHandle();
        });
        builder.SetVertexInputState([&](auto& vertexInputStateCreateInfo) {
            vertexInputStateCreateInfo.vertexBindingDescriptionCount = bindings.size();
            vertexInputStateCreateInfo.pVertexBindingDescriptions = bindings.data();
    ...
    if (!billboardingPipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (for billboarding objects)!");
    }
}
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Draw billboarding points
    currentCmdBuffer->BindPipeline(billboardingPipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
    scene_->TraverseOrdered(camera_->GetPosition(), [&](const SceneObject& sceneObject) {
        if (sceneObject.HasRenderable() && sceneObject.GetTag() == kBillboardGroup) {
            const auto [vertexOffsets, indexOffset, indexCount] = sceneObject.GetMeshGpu().value();
            currentCmdBuffer->BindVertexBuffers(vertexBuffers, 0, vertexBuffers.size(), vertexOffsets);
            currentCmdBuffer->BindIndexBuffer(scene_->GetGeometryBuffer(), indexOffset);
