...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto sceneGeometryShaderAsset = assetManager_->Load<ShaderAsset>(kSceneGeometryShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {{.name = kSceneVertexShaderKey, .asset = assetManager_->Get(sceneVertexShaderAsset)},
                    {.name = kSceneGeometryShaderKey, .asset = assetManager_->Get(sceneGeometryShaderAsset)},
...
void VulkanApplication::CreatePipelines()
{
    ...
    const auto halfWindowWidth = static_cast<float>(currentWindowWidth_ / 2.0);
    const auto halfWindowHeight = static_cast<float>(currentWindowHeight_ / 2.0);

    std::array viewports = {
        VkViewport{0, 0, halfWindowWidth, halfWindowHeight, 0.0f, 1.0f},
        VkViewport{halfWindowWidth, 0, halfWindowWidth, halfWindowHeight, 0.0f, 1.0f},
        VkViewport{0, halfWindowHeight, halfWindowWidth, halfWindowHeight, 0.0f, 1.0f},
        VkViewport{halfWindowWidth, halfWindowHeight, halfWindowWidth, halfWindowHeight, 0.0f, 1.0f}};

    const auto windowHalfSize = VkExtent2D{currentWindowWidth_ / 2U, currentWindowHeight_ / 2U};

    std::array scissors = {
        VkRect2D{0, 0, windowHalfSize},
        VkRect2D{static_cast<int32_t>(halfWindowWidth), 0, windowHalfSize},
        VkRect2D{0, static_cast<int32_t>(halfWindowHeight), windowHalfSize},
        VkRect2D{static_cast<int32_t>(halfWindowWidth), static_cast<int32_t>(halfWindowHeight), windowHalfSize}};
    ...
    scenePipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto& builder) {
        ...
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            ...
            shaderStageCreateInfo.module = resources_->GetShaderModule(kSceneGeometryShaderKey)->GetHandle();
        });
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
