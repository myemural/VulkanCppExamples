...
void VulkanApplication::CreateInitialResources() const
{
    ...
    resourceCreateInfo.buffers = {{kLightUniformBuffer, sizeof(LightUbo), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                  ...
                                  {kLightSpaceMatricesBuffer, sizeof(LightSpaceMatricesUBO),
    ...
    const auto shadowMapGeometryShaderAsset = assetManager_->Load<ShaderAsset>(kShadowMapGeometryShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {
            ...
            {.name = kShadowMapGeometryShaderKey, .asset = assetManager_->Get(shadowMapGeometryShaderAsset)},
    ...
    resourceCreateInfo.images = {
        ImageResourceCreateInfo{
            ...
            .arrayLayers = kCubemapFaceCount,
            .usageFlags = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            .views = {ImageViewCreateInfo{.viewName = kShadowCubemapImageView,
                                          .viewType = VK_IMAGE_VIEW_TYPE_CUBE,
                                          .format = depthImageFormat_,
                                          .subresourceRange{VK_IMAGE_ASPECT_DEPTH_BIT, 0, 1, 0, kCubemapFaceCount}},
                      ImageViewCreateInfo{.viewName = kShadowCubemapArrayView,
...
void VulkanApplication::BuildScene()
{
    ...
    roomMaterial.ambientStrength = kAmbientStrength;
    roomMaterial.shininess = kSpecularShininess;
    roomMaterial.specularStrength = kSpecularStrength;
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    std::vector<VkDescriptorBufferInfo> lightSpaceMatricesBufferInfos;
    lightSpaceMatricesBufferInfos.emplace_back(resources_->GetBuffer(kLightSpaceMatricesBuffer)->GetHandle(), 0,
                                               VK_WHOLE_SIZE);
    ...
    BufferWriteRequest shadowMapLightSpaceMatricesBufferRequest;
    shadowMapLightSpaceMatricesBufferRequest.descriptorSetName = kShadowMapDescSet;
    shadowMapLightSpaceMatricesBufferRequest.bindingIndex = 1;
    shadowMapLightSpaceMatricesBufferRequest.buffers = lightSpaceMatricesBufferInfos;
    shadowMapLightSpaceMatricesBufferRequest.type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    ...
    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        .bufferWriteRequests = {objectStorageTransformBufferRequest, objectStorageMaterialBufferRequest,
                                ...
                                shadowMapLightSpaceMatricesBufferRequest},
...
void VulkanApplication::CreatePipelines()
{
    ...
    shadowPipeline_ = device_->CreateGraphicsPipeline(shadowPipelineLayout_, shadowRenderPass_, [&](auto& builder) {
        ...
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            ...
            shaderStageCreateInfo.module = resources_->GetShaderModule(kShadowMapGeometryShaderKey)->GetHandle();
        });
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            shaderStageCreateInfo.stage = VK_SHADER_STAGE_FRAGMENT_BIT;
...
void VulkanApplication::CreateFramebuffers()
{
    ...
    const auto& shadowCubemapArrayImageView = resources_->GetImageView(kShadowCubemapImage, kShadowCubemapArrayView);

    shadowFramebuffer_ =
            device_->CreateFramebuffer(shadowRenderPass_, {shadowCubemapArrayImageView}, [&](auto& builder) {
                builder.SetDimensions(kShadowMapSize, kShadowMapSize, kCubemapFaceCount);
            });
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    LightSpaceMatricesUBO lightSpaceMatrices{};
    for (size_t i = 0; i < kCubemapFaceCount; ++i) {
        const glm::mat4 lightProjection = cubemapProj;
        const glm::mat4 lightView = cubemapViewMatrices[i];
        lightSpaceMatrices.lightSpaceMatrices[i] = lightProjection * lightView;
    }
    resources_->SetBuffer(kLightSpaceMatricesBuffer, &lightSpaceMatrices, sizeof(lightSpaceMatrices));
}
