...
void VulkanApplication::CreateInitialResources() const
{
    ...
    const auto mainGeometryShaderAsset = assetManager_->Load<ShaderAsset>(kMainGeometryShaderFile);
    ...
    resourceCreateInfo.shaders = {
        .modules = {
            ...
            {.name = kMainGeometryShaderKey, .asset = assetManager_->Get(mainGeometryShaderAsset)},
...
void VulkanApplication::BuildScene()
{
    ...
    for (const auto& pos: kObjectPositions) {
        defaultMaterial.diffuseColor = glm::vec4(GenerateRandomColor(), 1.0f);
        rootObjectBuilder.AddChild(SceneObjectBuilder(*scene_, kPointObject + std::to_string(index++))
...
void VulkanApplication::CreatePipelines()
{
    ...
    scenePipeline_ = device_->CreateGraphicsPipeline(pipelineLayout_, renderPass_, [&](auto& builder) {
        ...
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
            ...
            shaderStageCreateInfo.module = resources_->GetShaderModule(kMainGeometryShaderKey)->GetHandle();
        });
        builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    for (int i = 0; i < kTotalNoOfPositions; ++i) {
        constexpr float kSpacing = 0.4f;   // X distance between points
        constexpr float kAmplitude = 2.5f; // Wave length
        constexpr float kFrequency = 0.5f; // Wave frequency
        constexpr float kSpeed = 1.5f;     // Speed of the movement

        const float x = (static_cast<float>(i) - kTotalNoOfPositions * 0.5f) * kSpacing;
        const float y = kAmplitude * sin(kFrequency * static_cast<float>(i) + kSpeed * currentTime);
        constexpr float z = 0.0f;

        const std::string objectName = kPointObject + std::to_string(i);
        scene_->FindObjectByName(objectName)->SetPosition(glm::vec3(x, y, z));
    }
