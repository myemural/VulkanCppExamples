...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    meshPushConstants.triangleCount = currentTriangleCount_;
    currentCmdBuffer->PushConstants(pipelineLayout_, VK_SHADER_STAGE_MESH_BIT_EXT, 0,
                                    sizeof(meshPushConstants), &meshPushConstants);
    currentCmdBuffer->DrawMeshTasksEXT(1, 1, 1);
...
void VulkanApplication::ProcessInput()
{
    ...
    if (window_->IsKeyPressed(GLFW_KEY_1)) {
        currentTriangleCount_ = 1;
    }
    if (window_->IsKeyPressed(GLFW_KEY_2)) {
        currentTriangleCount_ = 2;
    }
    if (window_->IsKeyPressed(GLFW_KEY_3)) {
        currentTriangleCount_ = 3;
    }
}
} // namespace examples::advanced_shader_programming::mesh_and_task_shaders::triangles_with_mesh_shader
