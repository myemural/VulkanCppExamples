...
void VulkanApplication::CreateResources()
{
    ...
    suzanneMeshletLodData_ = suzanneModelAsset_->BuildMeshletWithLod(0, 0, kLodTargetRatios);
    totalMeshletCount_ = static_cast<std::uint32_t>(suzanneMeshletLodData_.meshlets.size());
    meshletTrianglesExpanded_.assign(suzanneMeshletLodData_.meshletTriangles.begin(),
                                     suzanneMeshletLodData_.meshletTriangles.end());

    ResourceDescriptor resourceCreateInfo;

    // Fill buffer create infos
    const auto vertexBufferSize =
            static_cast<std::uint32_t>(suzanneMeshletLodData_.vertices.size() * sizeof(VertexPos3Uv2));
    const auto meshletVertexIndicesSize =
            static_cast<std::uint32_t>(suzanneMeshletLodData_.meshletVertices.size() * sizeof(std::uint32_t));
    ...
    const auto meshletDescriptorBufferSize =
            static_cast<std::uint32_t>(suzanneMeshletLodData_.meshlets.size() * sizeof(MeshletDescriptor));
    const auto lodInfoBufferSize =
            static_cast<std::uint32_t>(suzanneMeshletLodData_.lodRanges.size() * sizeof(LodRange));
    ...
    resourceCreateInfo.buffers = {
        ...
        {kMeshletDescriptorBuffer, meshletDescriptorBufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT},
        {kLodInfoBuffer, lodInfoBufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT}};
...
{
    resources_->SetBuffer(kMeshletVertexBuffer, suzanneMeshletLodData_.vertices.data(),
                          static_cast<std::uint32_t>(suzanneMeshletLodData_.vertices.size() * sizeof(VertexPos3Uv2)));
    resources_->SetBuffer(
            kMeshletVertexIndicesBuffer, suzanneMeshletLodData_.meshletVertices.data(),
            static_cast<std::uint32_t>(suzanneMeshletLodData_.meshletVertices.size() * sizeof(std::uint32_t)));
    resources_->SetBuffer(kMeshletTriangleIndicesBuffer, meshletTrianglesExpanded_.data(),
                          static_cast<std::uint32_t>(meshletTrianglesExpanded_.size() * sizeof(std::uint32_t)));
    resources_->SetBuffer(
            kMeshletDescriptorBuffer, suzanneMeshletLodData_.meshlets.data(),
            static_cast<std::uint32_t>(suzanneMeshletLodData_.meshlets.size() * sizeof(MeshletDescriptor)));
    resources_->SetBuffer(kLodInfoBuffer, suzanneMeshletLodData_.lodRanges.data(),
                          static_cast<std::uint32_t>(suzanneMeshletLodData_.lodRanges.size() * sizeof(MeshletBounds)));
...
void VulkanApplication::InitTransformsAndCameras()
{
    ...
    const auto& randomObjectPositions = GenerateRandomPositions(kObjectCount, glm::vec3(-15.0f, -10.0f, -30.0f),
                                                                glm::vec3(15.0f, 10.0f, 4.0f), 3.0f);

    std::array<glm::vec3, kObjectCount> objectPositions = {glm::vec3(0.0f)};
    std::array<float, kObjectCount> objectRotationsY = {0.0f};
    for (auto i = 0U; i < kObjectCount; ++i) {
        objectPositions[i] = randomObjectPositions[i];
        objectRotationsY[i] = GenerateRandomValue(-90.0f, 90.0f);
    }

    for (auto i = 0U; i < kObjectCount; ++i) {
        auto model = glm::mat4(1.0f);
        model = glm::translate(model, objectPositions[i]);
        model = glm::rotate(model, glm::radians(objectRotationsY[i]), glm::vec3(0.0f, 1.0f, 0.0f));
...
void VulkanApplication::CreatePipelines()
{
    ...
    pushConstant.size = sizeof(LodPushConstants);
...
void VulkanApplication::UpdateDescriptorSets() const
{
    ...
    std::vector<VkDescriptorBufferInfo> storageMeshletLodInfoBufferInfos;
    storageMeshletLodInfoBufferInfos.emplace_back(resources_->GetBuffer(kLodInfoBuffer)->GetHandle(), 0, VK_WHOLE_SIZE);
    ...
    BufferWriteRequest bufferMeshletLodInfoRequest;
    bufferMeshletLodInfoRequest.descriptorSetName = kMainDescSet;
    bufferMeshletLodInfoRequest.bindingIndex = 4;
    bufferMeshletLodInfoRequest.buffers = storageMeshletLodInfoBufferInfos;
    bufferMeshletLodInfoRequest.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    for (const auto& modelMatrix: modelMatrices_) {
        LodPushConstants mainPushConstants{};
        mainPushConstants.modelMatrix = modelMatrix;
        mainPushConstants.projViewMatrix = mainViewProj;
        mainPushConstants.cameraPos = glm::vec4(camera_->GetPosition(), 1.0f);
        mainPushConstants.lodDistances = kLodDistances;
        currentCmdBuffer->PushConstants(pipelineLayout_, VK_SHADER_STAGE_TASK_BIT_EXT | VK_SHADER_STAGE_MESH_BIT_EXT, 0,
                                        sizeof(LodPushConstants), &mainPushConstants);
