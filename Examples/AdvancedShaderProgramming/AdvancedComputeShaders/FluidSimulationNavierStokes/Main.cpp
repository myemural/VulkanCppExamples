/**
 * @file    Main.cpp
 * @brief   This example demonstrates real-time GPU fluid simulation in Vulkan using compute shaders, implementing
 *          Navier-Stokes with semi-Lagrangian advection, Jacobi pressure projection, and buoyancy to produce a rising
 *          smoke plume in a 64x128x64 voxel grid. The result is rendered with volumetric ray marching for realistic
 *          lighting and shadowing of the smoke.
...
using namespace examples::advanced_shader_programming::advanced_compute_shaders::fluid_simulation_navier_stokes;
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::CameraZoomSpeed, 0.35f);
