/**
 * @file    Main.cpp
 * @brief   This example demonstrates a lightweight compute-shader wind simulation: An 8x8 grid of animated noise
 *          drives a wind field that gently sways each grass blade in real time. The wind volume and its per-cell
 *          direction/strength vectors can be visualized on demand with a single key toggle(G), rendering a debug box
 *          and directional arrows directly in the scene.
...
using namespace examples::advanced_shader_programming::advanced_compute_shaders::wind_simulation_foliage;
