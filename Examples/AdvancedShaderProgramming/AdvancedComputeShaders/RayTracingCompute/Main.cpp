/**
 * @file    Main.cpp
 * @brief   In this example, the whole scene is rendered by a compute shader that traverses a binned SAH Bounding
 *          Volume Hierarchy (BVH) built on the host from the scene objects. Primary rays, area light shadow rays and a
 *          single specular bounce are all traced in the same dispatch.
...
using namespace examples::advanced_shader_programming::advanced_compute_shaders::ray_tracing_compute;
