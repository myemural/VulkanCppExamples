/**
 * @file    Main.cpp
 * @brief   This example demonstrates a real-time N-body gravitational simulation of two colliding galaxies using a
 *          compute shader, computing particle positions and velocities in parallel on the GPU with a tile-based
 *          algorithm and ping-pong storage buffers. The resulting particles are rendered as additive-blended billboard
 *          sprites, viewed through an orbiting camera.
...
using namespace examples::advanced_shader_programming::advanced_compute_shaders::body_particle_compute;
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::ClearColor, VkClearColorValue{0.01f, 0.01f, 0.01f, 0.0f});
