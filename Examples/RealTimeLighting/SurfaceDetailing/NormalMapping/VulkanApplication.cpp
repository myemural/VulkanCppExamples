...
void VulkanApplication::BuildScene()
{
    ...
    // Add scene objects
    scene_->AddCube(kCubeObject, glm::vec3{-2.0f, -0.5f, 0.0f});
    scene_->SetMaterial(kCubeObject, defaultMatName);
    scene_->ScaleObject(kCubeObject, glm::vec3{2.0f});
    scene_->AddSphere(kSphereObject, glm::vec3{2.0f, -0.5f, 0.0f});
    scene_->SetMaterial(kSphereObject, defaultMatName);
    scene_->ScaleObject(kSphereObject, glm::vec3{2.0f});
    scene_->AddCone(kConeObject, glm::vec3{-2.0f, 2.5f, 0.0f});
    scene_->SetMaterial(kConeObject, defaultMatName);
    scene_->ScaleObject(kConeObject, glm::vec3{2.0f});
    scene_->AddCylinder(kCylinderObject, glm::vec3{2.0f, 2.5f, 0.0f});
    scene_->SetMaterial(kCylinderObject, defaultMatName);
    scene_->ScaleObject(kCylinderObject, glm::vec3{2.0f});
    scene_->AddPlane(kPlaneObjectBottom, glm::vec3{0.0f, -2.0f, 0.0f}, glm::vec3(0.0f), glm::vec3{4.0f});
    scene_->SetMaterial(kPlaneObjectBottom, defaultMatName);
    scene_->AddPlane(kPlaneObjectBack, glm::vec3{0.0f, 2.0f, -4.0f}, glm::vec3(90.0f, 0.0f, 0.0f), glm::vec3{4.0f});
    scene_->SetMaterial(kPlaneObjectBack, defaultMatName);
}
