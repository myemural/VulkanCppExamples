...
void VulkanApplication::BuildScene()
{
    ...
    // Materials
    materialManager_->LoadTexture(kMetalPatternTexture, kMainSampler, kMetalPatternTexturePath);
    materialManager_->LoadTexture(kMetalPatternNormalTexture, kMainSampler, kMetalPatternNormalTexturePath,
                                  VK_FORMAT_R8G8B8A8_UNORM);
    materialManager_->LoadTexture(kMetalPatternHeightTexture, kMainSampler, kMetalPatternHeightTexturePath,
                                  VK_FORMAT_R8G8B8A8_UNORM);
    ...
    materialManager_->CreatePhongTexturedMaterial(defaultMatName)
            ...
            .SetDiffuseMap(kMetalPatternTexture)
            .SetNormalMap(kMetalPatternNormalTexture)
            .SetHeightMap(kMetalPatternHeightTexture)
            .Build();

    // Add scene objects
    scene_->AddSphere(kSphereObject1, glm::vec3{-2.0f, -0.5f, 0.0f});
    scene_->SetMaterial(kSphereObject1, defaultMatName);
    scene_->ScaleObject(kSphereObject1, glm::vec3{2.0f});
    scene_->AddSphere(kSphereObject2, glm::vec3{2.0f, -0.5f, 0.0f});
    scene_->SetMaterial(kSphereObject2, defaultMatName);
    scene_->ScaleObject(kSphereObject2, glm::vec3{2.0f});
}
