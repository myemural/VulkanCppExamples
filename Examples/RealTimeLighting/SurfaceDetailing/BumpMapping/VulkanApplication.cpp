...
void VulkanApplication::BuildScene()
{
    ...
    materialManager_->LoadTexture(kWallStoneHeightTexture, kMainSampler, kWallStoneHeightTexturePath,
    ...
    materialManager_->CreatePhongTexturedMaterial(defaultMatName)
            ...
            .SetHeightMap(kWallStoneHeightTexture)
            .Build();
    ...
    for (auto i = 0; i < 3; ++i) {
        ...
        scene_->AddCube(kSphereObject + rowStr, glm::vec3{-0.5f, -1.0f, zShift});
        scene_->SetMaterial(kSphereObject + rowStr, defaultMatName);
        scene_->AddSphere(kConeObject + rowStr, glm::vec3{1.0f, -1.0f, zShift});
        scene_->SetMaterial(kConeObject + rowStr, defaultMatName);
        scene_->AddSphere(kCylinderObject + rowStr, glm::vec3{2.5f, -1.0f, zShift});
