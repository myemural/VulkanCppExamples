...
void VulkanApplication::BuildScene()
{
    ...
    // Add scene objects
    scene_->AddSphere(GetParamStr(AppConstants::SphereObject), glm::vec3{0.0f, 0.0f, 0.0f});
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        .maxSets = 2 + combinedImageSamplerCount + 1, // Plus 1 for skybox cubemap
    ...
    auto cubemapImageInfos = materialManager_->GetDescriptorImageInfo(GetParamStr(AppConstants::CubemapTexture));
