        ...
        ImageResourceCreateInfo{
            ...
            .views = {ImageViewCreateInfo{.viewName = GetParamStr(AppConstants::DepthImageView),
                                          ...
                                          .subresourceRange = {.aspectMask = VK_IMAGE_ASPECT_DEPTH_BIT,
                                                               ...
                                                               .layerCount = 1}}}},
        ImageResourceCreateInfo{
            .name = GetParamStr(AppConstants::ReflectionColorImage),
            .memProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
            .format = VK_FORMAT_R16G16B16A16_SFLOAT,
            .dimensions = {currentWindowWidth_, currentWindowHeight_, 1},
            .usageFlags = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
            .views = {ImageViewCreateInfo{.viewName = GetParamStr(AppConstants::ReflectionColorImageView),
                                          .format = VK_FORMAT_R16G16B16A16_SFLOAT}}},
        ImageResourceCreateInfo{
...
void VulkanApplication::BuildScene()
{
    ...
    const auto mirrorMatName = GetParamStr(AppConstants::MirrorMaterial);
    materialManager_->CreatePhongTexturedMaterial(mirrorMatName)
            .SetAmbientStrength(GetParamFloat(AppSettings::AmbientStrength))
            .SetSpecularStrength(GetParamFloat(AppSettings::SpecularStrength))
            .SetShininess(GetParamFloat(AppSettings::Shininess))
    ...
    // Add scene objects
    scene_->AddCube(GetParamStr(AppConstants::CubeObject), glm::vec3{-1.5f, -0.5f, 0.0f});
    scene_->SetMaterial(GetParamStr(AppConstants::CubeObject), defaultMatName);
    scene_->AddSphere(GetParamStr(AppConstants::SphereObject), glm::vec3{1.5f, -0.5f, 0.0f});
    ...
    // Add mirror object
    scene_->AddPlane(GetParamStr(AppConstants::MirrorObject), kMirrorObjectPosition, glm::vec3(90.0f, 0.0f, 0.0f),
                     glm::vec3{4.0f});
    scene_->SetMaterial(GetParamStr(AppConstants::MirrorObject), mirrorMatName);
...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    reflectionImageInfos.emplace_back(resources_->GetSampler(GetParamStr(AppConstants::MainSampler))->GetHandle(),
                                      resources_
                                              ->GetImageView(GetParamStr(AppConstants::ReflectionColorImage),
                                                             GetParamStr(AppConstants::ReflectionColorImageView))
                                              ->GetHandle(),
...
{
    const auto& reflectionImageView = resources_->GetImageView(GetParamStr(AppConstants::ReflectionColorImage),
                                                               GetParamStr(AppConstants::ReflectionColorImageView));
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    glm::mat4 reflectedView = camera_->GetReflectedViewMatrix(reflectionPlane);
    glm::mat4 proj = camera_->GetProjectionMatrix();
    glm::mat4 reflectionViewProj = camera_->GetReflectionViewProjMatrix(reflectionPlane);
    ...
    // Draw skybox
    {
        ...
        const auto meshPushConstants =
                meshInfo.GenerateMeshPushConstantsGpu(reflectedView, proj, glm::vec4(camera_->GetPosition(), 1.0f));
    ...
    for (const auto& [meshName, meshInfo]: scene_->GetAllMeshes()) {
        if (scene_->IsInGroup(meshName, GetParamStr(AppConstants::SkyboxObjectGroup)) ||
            meshName == GetParamStr(AppConstants::MirrorObject)) {
            continue;
        }
    ...
    // Draw skybox
    {
        ...
        const auto meshPushConstants = meshInfo.GenerateMeshPushConstantsGpu(
                view, proj, glm::vec4(camera_->GetPosition(), 1.0f), reflectionViewProj);
        currentCmdBuffer->PushConstants(pipelineLayoutSkybox_,
                                        VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                        sizeof(meshPushConstants), &meshPushConstants);
        currentCmdBuffer->DrawIndexed(indexCount, 1, 0, 0, 0);
    ...
    for (const auto& [meshName, meshInfo]: scene_->GetAllMeshes()) {
        ...
        auto meshPushConstants = meshInfo.GenerateMeshPushConstantsGpu(
                view, proj, glm::vec4(camera_->GetPosition(), 1.0f), reflectionViewProj);
        meshPushConstants.reflectionViewProjection = reflectionViewProj;
        currentCmdBuffer->PushConstants(pipelineLayout_, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                        sizeof(meshPushConstants), &meshPushConstants);
        currentCmdBuffer->DrawIndexed(indexCount, 1, 0, 0, 0);
    }
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    scene_->MoveObject(GetParamStr(AppConstants::CubeObject), glm::vec3(-1.5f, newY, 0.0f));
    scene_->MoveObject(GetParamStr(AppConstants::SphereObject), glm::vec3(1.5f, newY, 0.0f));
