...
void VulkanApplication::BuildScene()
{
    ...
    // Add light objects
    scene_->AddSphere(GetParamStr(AppConstants::LightObject1), glm::vec3{2.0f, 1.8f, -2.0f}, glm::vec3{0.0f},
                      glm::vec3{0.3f});
    scene_->AddSphere(GetParamStr(AppConstants::LightObject2), glm::vec3{-1.5f, 1.8f, 2.0f}, glm::vec3{0.0f},
                      glm::vec3{0.3f});
}
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    for (const auto& [meshName, meshInfo]: scene_->GetAllMeshes()) {
        ...
        if (meshName == GetParamStr(AppConstants::LightObject1) ||
            meshName == GetParamStr(AppConstants::LightObject2)) {
...
{
    const std::array lightMeshInfos{scene_->GetMesh(GetParamStr(AppConstants::LightObject1)),
                                    scene_->GetMesh(GetParamStr(AppConstants::LightObject2))};

    LightBlock lightBlock{};
    for (auto i = 0U; i < LIGHT_COUNT; ++i) {
        lightBlock.lightUbo[i].lightPosition = glm::vec4(lightMeshInfos[i].transform.translation, 1.0f);
        ...
        lightBlock.lightUbo[i].ambientParams.x = GetParamFloat(AppSettings::AmbientStrength);
        lightBlock.lightUbo[i].specularParams.x = GetParamFloat(AppSettings::SpecularStrength);
        lightBlock.lightUbo[i].specularParams.y = GetParamFloat(AppSettings::Shininess);
