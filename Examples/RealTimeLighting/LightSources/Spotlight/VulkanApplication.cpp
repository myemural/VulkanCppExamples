...
void VulkanApplication::CreateInitialResources() const
{
    ...
    resourceCreateInfo.buffers = {{kLightUniformBuffer, sizeof(LightBlock), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
...
void VulkanApplication::BuildScene()
{
    ...
    // Add light objects
    scene_->AddSphere(kLightObject1, glm::vec3{2.0f, 1.8f, -2.0f}, glm::vec3{0.0f}, glm::vec3{0.3f});
    scene_->AddSphere(kLightObject2, glm::vec3{-1.5f, 1.8f, 2.0f}, glm::vec3{0.0f}, glm::vec3{0.3f});
    scene_->AddToGroup(kLightGroup, {kLightObject1, kLightObject2});
}
...
{
    const std::array lightMeshInfos{scene_->GetMesh(kLightObject1), scene_->GetMesh(kLightObject2)};
    ...
    resources_->SetBuffer(kLightUniformBuffer, &lightBlock, sizeof(LightBlock));
}
