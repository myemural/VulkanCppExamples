...
void VulkanApplication::CreateInitialResources() const
{
    ...
    resourceCreateInfo.buffers = {{GetParamStr(AppConstants::LightStorageBuffer), sizeof(LightBuffer),
...
void VulkanApplication::BuildScene()
{
    ...
    // Add light objects
    scene_->AddCone(GetParamStr(AppConstants::SpotlightObject), glm::vec3{0.0f, 1.8f, 0.0f}, glm::vec3{0.0f},
                    glm::vec3{0.3f});
    scene_->AddSphere(GetParamStr(AppConstants::PointLightObject1), glm::vec3{-1.5f, 1.0f, 2.0f}, glm::vec3{0.0f},
                      glm::vec3{0.3f});
...
void VulkanApplication::UpdateDescriptorSets() const
{
    ...
    lightUboInfos.emplace_back(resources_->GetBuffer(GetParamStr(AppConstants::LightStorageBuffer))->GetHandle(), 0,
                               VK_WHOLE_SIZE);
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    const float angularSpeed = 0.6f * currentTime;
    constexpr float radius = 3.0f;
    const float x1 = radius * cos(angularSpeed);
    const float z1 = radius * sin(angularSpeed);
    scene_->MoveObject(GetParamStr(AppConstants::PointLightObject1), glm::vec3(x1, 1.0f, z1));
    ...
    scene_->MoveObject(GetParamStr(AppConstants::PointLightObject2), glm::vec3(x2, 1.0f, z2));
    ...
    lightBuffer.lights[1].lightPosition =
    ...
    resources_->SetBuffer(GetParamStr(AppConstants::LightStorageBuffer), &lightBuffer, sizeof(lightBuffer));
}
