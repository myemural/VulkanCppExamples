/**
 * @file    Main.cpp
 * @brief   In this example, objects of different colors are drawn on the scene, and lighting is provided by multiple
 *          lights which can be directional light, point light or spotlight (soft-cutoff). Light count provided to the
 *          shaders with a specialization constant.
...
using namespace examples::real_time_lighting::light_sources::multiple_light_sources;
...
inline ParameterSchema CreateParameterSchema()
{
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::LightStorageBuffer, "lightStorageBuffer");
    ...
    schema.RegisterImmutableParam<std::string>(AppConstants::SpotlightObject, "spotlight");
    ...
    schema.RegisterParam<float>(AppSettings::InnerCutoffAngle);
    schema.RegisterParam<float>(AppSettings::OuterCutoffAngle);
...
bool SetParams(ParameterServer& params)
{
    try {
        ...
        params.Set(AppSettings::InnerCutoffAngle, 12.0f);
        params.Set(AppSettings::OuterCutoffAngle, 25.0f);
        params.Set(AppSettings::ConstantFactor, 1.0f);
        params.Set(AppSettings::LinearFactor, 0.22f);
        params.Set(AppSettings::QuadraticFactor, 0.2f);
