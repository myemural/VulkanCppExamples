/**
 * @file    Main.cpp
 * @brief   In this example, gradient-based manual LOD control is applied via shader to objects drawn on the screen
 *          that have anisotropic filtering applied to their textures. Additionally, the diffuse texture is mixed with
 *          red color depending on the object's distance to the camera for showing the LOD level better.
...
using namespace examples::real_time_lighting::texture_sampling_and_filtering::gradient_based_mipmapping;
