...
void VulkanApplication::BuildScene()
{
    ...
    materialManager_->CreatePhongTexturedMaterial(defaultMatName)
            ...
            .SetDiffuseMap(kFloorTexture)
            .Build();

    // Add scene objects
    for (auto i = 0; i < modelPositions.size(); ++i) {
        const std::string indexStr = std::to_string(i);
        scene_->AddSphere(kSphereObject + indexStr, modelPositions[i]);
        scene_->SetMaterial(kSphereObject + indexStr, defaultMatName);
    }

    scene_->AddPlane(kPlaneObject, glm::vec3{0.0f, -7.0f, -8.0f}, glm::vec3(0.0f), glm::vec3{12.0f});
