...
void VulkanApplication::BuildScene()
{
    ...
    materialManager_->LoadTexture(kRockTexture, kMainSampler, assetManager_->Get(rockTextureAsset));
    const auto rockSpecTextureAsset = assetManager_->Load<TextureAsset>(kRockSpecTexturePath);
    materialManager_->LoadTexture(kRockSpecTexture, kMainSampler, assetManager_->Get(rockSpecTextureAsset),
                                  VK_FORMAT_R8G8B8A8_UNORM);
    const auto rockRoughTextureAsset = assetManager_->Load<TextureAsset>(kRockRoughTexturePath);
    materialManager_->LoadTexture(kRockRoughTexture, kMainSampler, assetManager_->Get(rockRoughTextureAsset),
                                  VK_FORMAT_R8G8B8A8_UNORM);
    ...
    defaultMaterial.diffuseMap = materialManager_->GetTextureId(kRockTexture);
    defaultMaterial.specularMap = materialManager_->GetTextureId(kRockSpecTexture);
    defaultMaterial.roughnessMap = materialManager_->GetTextureId(kRockRoughTexture);
