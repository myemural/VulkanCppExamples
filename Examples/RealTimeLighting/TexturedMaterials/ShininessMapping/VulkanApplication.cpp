...
void VulkanApplication::BuildScene()
{
    ...
    // Materials
    const auto rockTextureAsset = assetManager_->Load<TextureAsset>(kRockTexturePath);
    const auto rockTextureId =
            sceneImageStorage.StoreTexture(kRockTexture, kMainSampler, assetManager_->Get(rockTextureAsset));
    const auto rockSpecTextureAsset = assetManager_->Load<TextureAsset>(kRockSpecTexturePath);
    const auto rockSpecTextureId = sceneImageStorage.StoreTexture(
            kRockSpecTexture, kMainSampler, assetManager_->Get(rockSpecTextureAsset), VK_FORMAT_R8G8B8A8_UNORM);
    const auto rockRoughTextureAsset = assetManager_->Load<TextureAsset>(kRockRoughTexturePath);
    const auto rockRoughTextureId = sceneImageStorage.StoreTexture(
            kRockRoughTexture, kMainSampler, assetManager_->Get(rockRoughTextureAsset), VK_FORMAT_R8G8B8A8_UNORM);
    ...
    defaultMaterial.diffuseMap = rockTextureId;
    defaultMaterial.specularMap = rockSpecTextureId;
    defaultMaterial.roughnessMap = rockRoughTextureId;
...
void VulkanApplication::UpdateSceneTransforms() const
{
    ...
    scene_->FindObjectByName(kLightObject)->SetPosition(glm::vec3(x, 0.8f, z));
