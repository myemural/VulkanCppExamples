...
void VulkanApplication::CreateAndUpdateDescriptorSets() const
{
    ...
    const DescriptorResourceCreateInfo descriptorResourceCreateInfo = {
        ...
        .layouts =
                {{.name = kMainDescSetLayout,
                 ...
                 {.name = kTransparencyDescSetLayout,
                  .bindings = {{0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT, nullptr},
                               {1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_FRAGMENT_BIT, nullptr},
                               {2, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, combinedImageSamplerCount,
                                VK_SHADER_STAGE_FRAGMENT_BIT, nullptr},
        ...
        .descriptorSets = {{.name = kMainDescSet, .layoutName = kMainDescSetLayout},
                           ...
                           {.name = kTransparencyDescSet, .layoutName = kTransparencyDescSetLayout}}};
    ...
    BufferWriteRequest transparencyTransformStorageBufferRequest;
    transparencyTransformStorageBufferRequest.descriptorSetName = kTransparencyDescSet;
    transparencyTransformStorageBufferRequest.bindingIndex = 0;
    transparencyTransformStorageBufferRequest.buffers = storageTransformBufferInfos;
    transparencyTransformStorageBufferRequest.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;

    BufferWriteRequest transparencyMaterialBufferRequest;
    transparencyMaterialBufferRequest.descriptorSetName = kTransparencyDescSet;
    transparencyMaterialBufferRequest.bindingIndex = 1;
    transparencyMaterialBufferRequest.buffers = storageMaterialBufferInfos;
    transparencyMaterialBufferRequest.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;

    ImageWriteRequest transparencyTextureUpdateRequest;
    transparencyTextureUpdateRequest.descriptorSetName = kTransparencyDescSet;
    transparencyTextureUpdateRequest.bindingIndex = 2;
    transparencyTextureUpdateRequest.images = descriptorImageInfos;
    transparencyTextureUpdateRequest.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;

    BufferWriteRequest transparencyLightUpdateRequest;
    transparencyLightUpdateRequest.descriptorSetName = kTransparencyDescSet;
    transparencyLightUpdateRequest.bindingIndex = 3;
    transparencyLightUpdateRequest.buffers = storageLightBufferInfos;
    transparencyLightUpdateRequest.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;

    const DescriptorUpdateInfo descriptorSetUpdateInfo = {
        .bufferWriteRequests = {transformStorageBufferRequest, transformMaterialBufferRequest, lightUpdateRequest,
                                transparencyTransformStorageBufferRequest, transparencyMaterialBufferRequest,
                                transparencyLightUpdateRequest},
        .imageWriteRequests = {textureUpdateRequest, positionUpdateRequest, albedoUpdateRequest, normalUpdateRequest,
                               transparencyTextureUpdateRequest}};
...
void VulkanApplication::CreateRenderPass()
{
    ...
    VkAttachmentReference lightPassDepthAttachmentRef{1, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL};
    ...
    lightRenderPass_ = device_->CreateRenderPass([&](auto& builder) {
        builder.AddAttachment([](auto& attachmentCreateInfo) {
                ...
                .AddSubpass([&](auto& subpassCreateInfo) {
                    ...
                    subpassCreateInfo.pDepthStencilAttachment = &lightPassDepthAttachmentRef;
                });
    });
...
void VulkanApplication::CreatePipelines()
{
    ...
    transparencyPipelineLayout_ = device_->CreatePipelineLayout(
            {resources_->GetDescriptorLayout(kTransparencyDescSetLayout)}, {mvpPushConstant});

    if (!transparencyPipelineLayout_) {
        throw std::runtime_error("Failed to create pipeline layout!");
    }

    VkPipelineColorBlendAttachmentState transparencyColorBlendAttachment;
    transparencyColorBlendAttachment.blendEnable = VK_TRUE;
    transparencyColorBlendAttachment.srcColorBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA;
    transparencyColorBlendAttachment.dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
    transparencyColorBlendAttachment.colorBlendOp = VK_BLEND_OP_ADD;
    transparencyColorBlendAttachment.srcAlphaBlendFactor = VK_BLEND_FACTOR_ONE;
    transparencyColorBlendAttachment.dstAlphaBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
    transparencyColorBlendAttachment.alphaBlendOp = VK_BLEND_OP_ADD;
    transparencyColorBlendAttachment.colorWriteMask =
            VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT | VK_COLOR_COMPONENT_A_BIT;

    transparencyPipeline_ =
            device_->CreateGraphicsPipeline(transparencyPipelineLayout_, lightRenderPass_, [&](auto& builder) {
                builder.AddShaderStage([&](auto& shaderStageCreateInfo) {
                    shaderStageCreateInfo.stage = VK_SHADER_STAGE_VERTEX_BIT;
                    shaderStageCreateInfo.module = resources_->GetShaderModule(kGeometryVertexShaderKey)->GetHandle();
                });
                ...
                builder.SetColorBlendState([&](auto& blendStateCreateInfo) {
                    blendStateCreateInfo.attachmentCount = 1;
                    blendStateCreateInfo.pAttachments = &transparencyColorBlendAttachment;
                });
                builder.SetDepthStencilState([&](auto& depthStencilStateCreateInfo) {
                    depthStencilStateCreateInfo.depthTestEnable = VK_TRUE;
                    depthStencilStateCreateInfo.depthWriteEnable = VK_FALSE;
    ...
    if (!transparencyPipeline_) {
        throw std::runtime_error("Failed to create graphics pipeline (for transparent objects)!");
    }
}
...
void VulkanApplication::CreateFramebuffers()
{
    ...
    for (const auto& swapImage: swapChainImageViews_) {
        auto framebuffer =
                device_->CreateFramebuffer(lightRenderPass_, {swapImage, depthImageView}, [&](auto& builder) {
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    // Light Pass
    {
        ...
        // Lighting for opaque objects
        {
            ...
            currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, lightPipelineLayout_, 0,
                                                 lightDescSets);
        ...
        // Drawing and lighting for transparent objects
        {
            const std::vector transparencyDescSets{resources_->GetDescriptorSet(kTransparencyDescSet)};
            currentCmdBuffer->BindDescriptorSets(VK_PIPELINE_BIND_POINT_GRAPHICS, transparencyPipelineLayout_, 0,
                                                 transparencyDescSets);
            const std::vector vertexBuffers(scene_->GetAttributeCount(), scene_->GetGeometryBuffer());
            currentCmdBuffer->BindPipeline(transparencyPipeline_, VK_PIPELINE_BIND_POINT_GRAPHICS);
            scene_->TraverseOrdered(camera_->GetPosition(), [&](const SceneObject& sceneObject) {
                if (sceneObject.HasRenderable() && sceneObject.GetTag() == kTransparentObjectGroup) {
                    ...
                    meshPushConstants.view = camera_->GetViewMatrix();
                    meshPushConstants.projection = camera_->GetProjectionMatrix();
                    currentCmdBuffer->PushConstants(transparencyPipelineLayout_,
                                                    VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                                    sizeof(meshPushConstants), &meshPushConstants);
                    currentCmdBuffer->DrawIndexed(indexCount, 1, 0, 0, 0);
                }
