...
void VulkanApplication::CreateInitialResources()
{
    ...
    const auto surfaceFormat = physicalDevice_->GetSurfaceFormat(surface_->GetHandle(), VK_FORMAT_B8G8R8A8_SRGB,
                                                                 VK_COLOR_SPACE_SRGB_NONLINEAR_KHR);

    // Create multisample supported color and depth images
    resourceCreateInfo.images = std::vector<ImageResourceCreateInfo>{};
    for (auto i = 0; i < swapChainImageViews_.size(); ++i) {
        const std::string colorImageName = kResolveImage + std::string("_") + std::to_string(i);
        const std::string colorImageViewName = kResolveImageView + std::string("_") + std::to_string(i);
        const std::string depthImageName = kDepthImage + std::string("_") + std::to_string(i);
        const std::string depthImageViewName = kDepthImageView + std::string("_") + std::to_string(i);
        ...
        resourceCreateInfo.images->emplace_back(ImageResourceCreateInfo{
            ...
            .views = {ImageViewCreateInfo{.viewName = colorImageViewName,
                                          .format = surfaceFormat->format,
        ...
        resourceCreateInfo.images->emplace_back(ImageResourceCreateInfo{
            ...
            .views = {ImageViewCreateInfo{.viewName = depthImageViewName,
                                          .format = depthImageFormat_,
...
void VulkanApplication::BuildScene()
{
    ...
    camera_ = std::make_shared<PerspectiveCamera>(glm::vec3(0.0f, 2.0f, 3.0f), aspectRatio);
    ...
    const auto rootObject = SceneObjectBuilder(*scene_, kRootObject)
                                    ...
                                    .AddChild(SceneObjectBuilder(*scene_, kWallObject1)
                                                      .WithBuiltinMesh(BuiltinMeshType::CUBE)
                                                      .WithMaterial(wallStoneMaterial)
                                                      .WithPosition(glm::vec3{-2.5f, 2.0f, -2.0f})
                                                      .WithScale(glm::vec3{1.0f, 2.0f, 1.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kWallObject2)
                                                      .WithBuiltinMesh(BuiltinMeshType::CUBE)
                                                      .WithMaterial(wallStoneMaterial)
                                                      .WithPosition(glm::vec3{2.5f, 2.0f, -2.0f})
                                                      .WithScale(glm::vec3{1.0f, 2.0f, 1.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kWallObject3)
                                                      .WithBuiltinMesh(BuiltinMeshType::CUBE)
                                                      .WithMaterial(wallStoneMaterial)
                                                      .WithPosition(glm::vec3{-2.5f, 2.0f, -6.0f})
                                                      .WithScale(glm::vec3{1.0f, 2.0f, 1.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kWallObject4)
                                                      .WithBuiltinMesh(BuiltinMeshType::CUBE)
                                                      .WithMaterial(wallStoneMaterial)
                                                      .WithPosition(glm::vec3{2.5f, 2.0f, -6.0f})
                                                      .WithScale(glm::vec3{1.0f, 2.0f, 1.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kWallObject5)
                                                      .WithBuiltinMesh(BuiltinMeshType::CUBE)
                                                      .WithMaterial(wallStoneMaterial)
                                                      .WithPosition(glm::vec3{-2.5f, 2.0f, -10.0f})
                                                      .WithScale(glm::vec3{1.0f, 2.0f, 1.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kWallObject6)
                                                      .WithBuiltinMesh(BuiltinMeshType::CUBE)
                                                      .WithMaterial(wallStoneMaterial)
                                                      .WithPosition(glm::vec3{2.5f, 2.0f, -10.0f})
                                                      .WithScale(glm::vec3{1.0f, 2.0f, 1.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kBottomPlane)
                                                      .WithBuiltinMesh(BuiltinMeshType::PLANE)
                                                      .WithMaterial(wallStoneMaterial)
                                                      .WithPosition(glm::vec3{0.0f, 1.0f, -6.0f})
                                                      .WithScale(glm::vec3{6.0f, 1.0f, 12.0f}))
                                    .AddChild(SceneObjectBuilder(*scene_, kRedPlane)
                                                      .WithBuiltinMesh(BuiltinMeshType::PLANE)
                                                      .WithMaterial(metalChainMaterial)
                                                      .WithPosition(glm::vec3{0.0f, 2.0f, -2.0f})
                                                      .WithEulerAngles(glm::vec3(90.0f, 0.0f, 0.0f))
                                                      .WithScale(glm::vec3{4.0f, 1.0f, 2.0f}))
...
void VulkanApplication::CreateRenderPass()
{
    ...
    VkAttachmentReference colorAttachmentResolvedRef{1, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};
    ...
    renderPass_ = device_->CreateRenderPass([&](auto& builder) {
        builder.AddAttachment([&](auto& attachmentCreateInfo) {
                ...
                .AddAttachment([](auto& attachmentCreateInfo) {
                    ...
                    attachmentCreateInfo.loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
                ...
                .AddSubpass([&](auto& subpassCreateInfo) {
                    ...
                    subpassCreateInfo.pResolveAttachments = &colorAttachmentResolvedRef;
...
void VulkanApplication::CreateFramebuffers()
{
    ...
    for (auto i = 0; i < swapChainImageViews_.size(); ++i) {
        const auto currentResolveImageView =
                resources_->GetImageView(kResolveImage + std::string("_") + std::to_string(i),
                                         kResolveImageView + std::string("_") + std::to_string(i));

        const auto currentDepthImageView =
                resources_->GetImageView(kDepthImage + std::string("_") + std::to_string(i),
                                         kDepthImageView + std::string("_") + std::to_string(i));
        auto framebuffer = device_->CreateFramebuffer(
                renderPass_, {currentResolveImageView, swapChainImageViews_.at(i), currentDepthImageView},
                [&](auto& builder) { builder.SetDimensions(windowWidth, windowHeight); });
...
void VulkanApplication::RecordPresentCommandBuffers(const std::uint32_t currentImageIndex)
{
    ...
    clearValues[1].color = params_.Get<VkClearColorValue>(AppSettings::ClearColor);
